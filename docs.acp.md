...
This document describes how the TheiaOS ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`theiaos acp` exposes an ACP agent over stdio and forwards prompts to a running
TheiaOS Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a TheiaOS Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `theiaos acp` over stdio.

Example config:

```bash
theiaos config set gateway.remote.url wss://gateway-host:18789
theiaos config set gateway.remote.token <token>
```

Example run:

```bash
theiaos acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
theiaos acp --session agent:main:main
theiaos acp --session agent:design:main
theiaos acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “TheiaOS ACP” to start a thread.

## Execution Model

- ACP client spawns `theiaos acp` and speaks ACP messages over stdio.
...
```bash
theiaos acp --session agent:main:main
theiaos acp --session-label "support inbox"
theiaos acp --reset-session
```
...
`theiaos acp` resolves the Gateway URL and auth from CLI flags or config:
