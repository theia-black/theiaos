...
export async function handleWhatsAppStart(host: TheiaOSApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: TheiaOSApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: TheiaOSApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: TheiaOSApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: TheiaOSApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: TheiaOSApp): string {
...
function resolveGatewayHttpAuthHeader(host: TheiaOSApp): string | null {
...
function buildGatewayHttpHeaders(host: TheiaOSApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: TheiaOSApp) {
...
export function handleNostrProfileToggleAdvanced(host: TheiaOSApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: TheiaOSApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: TheiaOSApp) {
  const state = host.nostrProfileFormState;
