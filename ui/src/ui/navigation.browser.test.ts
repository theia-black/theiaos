...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/theiaos/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/theiaos");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/theiaos/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__THEIAOS_CONTROL_UI_BASE_PATH__ = "/theiaos";
    const app = mountApp("/theiaos/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/theiaos");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/theiaos/sessions");
  });
