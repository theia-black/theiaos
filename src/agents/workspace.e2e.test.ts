...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses THEIAOS_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      THEIAOS_HOME: "/srv/theiaos-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/theiaos-home"), ".theiaos", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".theiaos", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("theiaos-workspace-");
