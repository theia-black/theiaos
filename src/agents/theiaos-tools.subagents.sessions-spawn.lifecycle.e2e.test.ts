...
} from "./theiaos-tools.subagents.sessions-spawn.test-harness.js";
...
type CreateTheiaOSTools = (typeof import("./theiaos-tools.js"))["createTheiaOSTools"];
type CreateTheiaOSToolsOpts = Parameters<CreateTheiaOSTools>[0];

async function getSessionsSpawnTool(opts: CreateTheiaOSToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createTheiaOSTools } = await import("./theiaos-tools.js");
  const tool = createTheiaOSTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
...
describe("theiaos-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
  });
