...
function resolveExecConfig(params: { cfg?: TheiaOSConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: TheiaOSConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createTheiaOSCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createTheiaOSReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
