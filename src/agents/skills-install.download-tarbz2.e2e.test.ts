...
async function withTempWorkspace(
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "theiaos-skills-install-"));
  try {
...
function restoreTheiaOSStateDir(originalValue: string | undefined): void {
  if (originalValue === undefined) {
    delete process.env.THEIAOS_STATE_DIR;
    return;
  }
  process.env.THEIAOS_STATE_DIR = originalValue;
}

const originalStateDir = process.env.THEIAOS_STATE_DIR;

afterEach(() => {
  restoreTheiaOSStateDir(originalStateDir);
});
