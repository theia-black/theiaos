...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("theiaos-ws-", async (workspaceDir) => {
      await withTempDir("theiaos-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createTheiaOSCodingTools({ workspaceDir });
    ...
    await withTempDir("theiaos-ws-", async (workspaceDir) => {
      await withTempDir("theiaos-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello theiaos");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("theiaos-ws-", async (workspaceDir) => {
      await withTempDir("theiaos-override-", async (overrideDir) => {
        const tools = createTheiaOSCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("theiaos-sandbox-", async (sandboxDir) => {
      await withTempDir("theiaos-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          containerName: "theiaos-sbx-test",
        ...
        const tools = createTheiaOSCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
