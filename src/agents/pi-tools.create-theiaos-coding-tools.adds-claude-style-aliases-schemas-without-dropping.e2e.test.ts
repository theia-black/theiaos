...
import { createTheiaOSTools } from "./theiaos-tools.js";
import { __testing, createTheiaOSCodingTools } from "./pi-tools.js";
import { createTheiaOSReadTool, createSandboxedReadTool } from "./pi-tools.read.js";
import { createHostSandboxFsBridge } from "./test-helpers/host-sandbox-fs-bridge.js";
import { createBrowserTool } from "./tools/browser-tool.js";

const defaultTools = createTheiaOSCodingTools();
...
describe("createTheiaOSCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createTheiaOSTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createTheiaOSCodingTools({ messageProvider: "discord" });
  ...
  it("filters session tools for sub-agent sessions by default", () => {
    const tools = createTheiaOSCodingTools({
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "theiaos-depth-policy-"));
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "theiaos-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "theiaos-outside.txt");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "theiaos-read-autopage-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "theiaos-read-cap-"));
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createTheiaOSReadTool(
      baseRead as unknown as Parameters<typeof createTheiaOSReadTool>[0],
    );
