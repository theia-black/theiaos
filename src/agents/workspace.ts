...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.THEIAOS_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".theiaos", `workspace-${profile}`);
  }
  return path.join(home, ".theiaos", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".theiaos";
