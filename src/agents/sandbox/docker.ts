...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "theiaos.sandbox=1");
  args.push("--label", `theiaos.sessionKey=${params.scopeKey}`);
  args.push("--label", `theiaos.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `theiaos.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "theiaos.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`theiaos sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`theiaos sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("theiaos sandbox recreate --all");
}
