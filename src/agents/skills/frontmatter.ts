...
import {
  ...
  normalizeStringList,
  parseTheiaOSManifestInstallBase,
  parseFrontmatterBool,
  resolveTheiaOSManifestBlock,
  resolveTheiaOSManifestInstall,
  resolveTheiaOSManifestOs,
  resolveTheiaOSManifestRequires,
} from "../../shared/frontmatter.js";
import type {
  TheiaOSSkillMetadata,
...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseTheiaOSManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
...
export function resolveTheiaOSMetadata(
  frontmatter: ParsedSkillFrontmatter,
): TheiaOSSkillMetadata | undefined {
  const metadataObj = resolveTheiaOSManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveTheiaOSManifestRequires(metadataObj);
  const install = resolveTheiaOSManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveTheiaOSManifestOs(metadataObj);
  return {
