...
type WebSearchConfig = NonNullable<TheiaOSConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: TheiaOSConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("theiaos configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.theiaos.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://theiaos.ai",
      "X-Title": "TheiaOS Web Search",
    },
