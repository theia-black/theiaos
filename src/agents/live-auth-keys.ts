...
const GOOGLE_LIVE_SINGLE_KEY = "THEIAOS_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "THEIAOS_LIVE_ANTHROPIC_KEY",
    listVar: "THEIAOS_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "THEIAOS_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `THEIAOS_LIVE_${base}_KEY`;
