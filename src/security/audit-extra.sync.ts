...
function summarizeGroupPolicy(cfg: TheiaOSConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: TheiaOSConfig): boolean {
...
function collectModels(cfg: TheiaOSConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: TheiaOSConfig): Set<string> {
  const baseCfg: TheiaOSConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: TheiaOSConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: TheiaOSConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: TheiaOSConfig): boolean {
...
function isBrowserEnabled(cfg: TheiaOSConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: TheiaOSConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: TheiaOSConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep THEIAOS_STATE_DIR on a local-only volume and re-run "${formatCliCommand("theiaos security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: TheiaOSConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer THEIAOS_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const theiaosGatewayToken =
    typeof env.THEIAOS_GATEWAY_TOKEN === "string" && env.THEIAOS_GATEWAY_TOKEN.trim()
      ? env.THEIAOS_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-theiaos-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: TheiaOSConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: TheiaOSConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
...
export function collectNodeDenyCommandPatternFindings(cfg: TheiaOSConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: TheiaOSConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: TheiaOSConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: TheiaOSConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
