...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".theiaos");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.THEIAOS_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$THEIAOS_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via THEIAOS_STATE_DIR.
