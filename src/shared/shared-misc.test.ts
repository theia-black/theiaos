...
describe("shared/frontmatter", () => {
  ...
  test("resolveTheiaOSManifestBlock parses JSON5 metadata and picks theiaos block", () => {
    const frontmatter = {
      metadata: "{ theiaos: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveTheiaOSManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveTheiaOSManifestBlock returns undefined for invalid input", () => {
    expect(resolveTheiaOSManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveTheiaOSManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveTheiaOSManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
