...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of TheiaOS")).toBe("friends-of-theiaos");
  ...
  it("matches ids or names", () => {
    const allow = normalizeDiscordAllowList(
      ["123", "steipete", "Friends of TheiaOS"],
    ...
    expect(allowListMatches(allow, { name: "friends-of-theiaos" })).toBe(true);
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-theiaos" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of TheiaOS"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-theiaos");
  });

  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-theiaos": { slug: "friends-of-theiaos" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of TheiaOS"),
      guildEntries,
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["theiaos-dm"],
        channelId: "1",
        channelName: "TheiaOS DM",
        channelSlug: "theiaos-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({
