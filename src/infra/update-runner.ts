...
const DEFAULT_PACKAGE_NAME = "theiaos";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "theiaos-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "theiaos.mjs");
    ...
    const doctorStep = await runStep(
      step("theiaos doctor", doctorArgv, gitRoot, { THEIAOS_UPDATE_IN_PROGRESS: "1" }),
    );
