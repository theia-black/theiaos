...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: TheiaOSConfig | null = null;
  ...
  get(key: string, cfg: TheiaOSConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: TheiaOSConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: TheiaOSConfig): void {
