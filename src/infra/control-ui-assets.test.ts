...
vi.mock("./theiaos-root.js", () => ({
  resolveTheiaOSPackageRoot: vi.fn(async () => null),
  resolveTheiaOSPackageRootSync: vi.fn(() => null),
}));
...
describe("control UI assets helpers (fs-mocked)", () => {
  ...
  it("uses resolveTheiaOSPackageRoot when available", async () => {
    const theiaosRoot = await import("./theiaos-root.js");
    const { resolveControlUiDistIndexPath } = await import("./control-ui-assets.js");

    const pkgRoot = abs("fixtures/theiaos");
    (
      theiaosRoot.resolveTheiaOSPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/theiaos"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "theiaos" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "theiaos.mjs"))).resolves.toBe(
  ...
  it("returns null when fallback package name does not match", async () => {
    ...
    const root = abs("fixtures/not-theiaos");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    ...
    const pkgRoot = abs("fixtures/theiaos-bundle");
    (
      theiaosRoot.resolveTheiaOSPackageRootSync as unknown as ReturnType<typeof vi.fn>
