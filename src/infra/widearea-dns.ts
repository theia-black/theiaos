...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.THEIAOS_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*theiaos-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "theiaos";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "theiaos");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "theiaos-gw");
  ...
  records.push(`_theiaos-gw._tcp IN PTR ${instanceLabel}._theiaos-gw._tcp`);
  records.push(`${instanceLabel}._theiaos-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._theiaos-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; theiaos-content-hash: ${contentHash}\n${contentBody}`;
}
