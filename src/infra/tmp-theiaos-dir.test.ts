...
import { POSIX_THEIAOS_TMP_DIR, resolvePreferredTheiaOSTmpDir } from "./tmp-theiaos-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredTheiaOSTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `theiaos-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredTheiaOSTmpDir({
    accessSync,
...
describe("resolvePreferredTheiaOSTmpDir", () => {
  it("prefers /tmp/theiaos when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_THEIAOS_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/theiaos when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_THEIAOS_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/theiaos when /tmp/theiaos is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/theiaos when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/theiaos is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/theiaos is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/theiaos is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
