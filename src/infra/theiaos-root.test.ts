...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__theiaos_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "theiaos-root");
...
describe("resolveTheiaOSPackageRoot", () => {
  beforeEach(() => {
    state.entries.clear();
  ...
  it("resolves package root from .bin argv1", async () => {
    const { resolveTheiaOSPackageRootSync } = await import("./theiaos-root.js");

    const project = fx("bin-scenario");
    const argv1 = path.join(project, "node_modules", ".bin", "theiaos");
    const pkgRoot = path.join(project, "node_modules", "theiaos");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "theiaos" }));

    expect(resolveTheiaOSPackageRootSync({ argv1 })).toBe(pkgRoot);
  });
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    const bin = path.join(project, "bin", "theiaos");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "theiaos.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "theiaos" }));

    expect(resolveTheiaOSPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveTheiaOSPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-theiaos package roots", async () => {
    const { resolveTheiaOSPackageRootSync } = await import("./theiaos-root.js");

    const pkgRoot = fx("not-theiaos");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-theiaos" }));

    expect(resolveTheiaOSPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });

  it("async resolver matches sync behavior", async () => {
    const { resolveTheiaOSPackageRoot } = await import("./theiaos-root.js");

    const pkgRoot = fx("async");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "theiaos" }));

    await expect(resolveTheiaOSPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });
});
