...
describe("resolveEffectiveHomeDir", () => {
  it("prefers THEIAOS_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands THEIAOS_HOME when set to ~", () => {
    const env = {
      THEIAOS_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for THEIAOS_HOME", () => {
    const result = resolveRequiredHomeDir(
      { THEIAOS_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when THEIAOS_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ THEIAOS_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { THEIAOS_HOME: "/srv/theiaos-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/theiaos-home")}/x`);
  });
