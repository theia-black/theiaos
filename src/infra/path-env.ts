...
function candidateBinDirs(opts: EnsureTheiaOSPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `theiaos` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "theiaos");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.THEIAOS_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "theiaos"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `theiaos` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureTheiaOSCliOnPath(opts: EnsureTheiaOSPathOpts = {}) {
  if (isTruthyEnvValue(process.env.THEIAOS_PATH_BOOTSTRAPPED)) {
    return;
  }
