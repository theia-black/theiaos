...
import { resolveTheiaOSPackageRoot, resolveTheiaOSPackageRootSync } from "./theiaos-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveTheiaOSPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "theiaos" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveTheiaOSPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
