...
export function triggerTheiaOSRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.THEIAOS_SYSTEMD_UNIT,
        process.env.THEIAOS_PROFILE,
      );
  ...
  const label =
    process.env.THEIAOS_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.THEIAOS_PROFILE);
