...
let ensureTheiaOSCliOnPath: typeof import("./path-env.js").ensureTheiaOSCliOnPath;

describe("ensureTheiaOSCliOnPath", () => {
  const envKeys = [
    "PATH",
    "THEIAOS_PATH_BOOTSTRAPPED",
    "THEIAOS_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureTheiaOSCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling theiaos exists", () => {
    const tmp = abs("/tmp/theiaos-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "theiaos");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.THEIAOS_PATH_BOOTSTRAPPED;

    ensureTheiaOSCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.THEIAOS_PATH_BOOTSTRAPPED = "1";
    ensureTheiaOSCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/theiaos-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "theiaos");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/theiaos-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "theiaos");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "theiaos");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/theiaos-path/case-linuxbrew");
