...
describe("update-startup", () => {
  ...
  let resolveTheiaOSPackageRoot: (typeof import("./theiaos-root.js"))["resolveTheiaOSPackageRoot"];
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "theiaos-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    hadStateDir = Object.prototype.hasOwnProperty.call(process.env, "THEIAOS_STATE_DIR");
    prevStateDir = process.env.THEIAOS_STATE_DIR;
    process.env.THEIAOS_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveTheiaOSPackageRoot } = await import("./theiaos-root.js"));
  ...
  async function runUpdateCheckAndReadState(channel: "stable" | "beta") {
    vi.mocked(resolveTheiaOSPackageRoot).mockResolvedValue("/opt/theiaos");
