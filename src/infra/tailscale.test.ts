...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.THEIAOS_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.THEIAOS_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.THEIAOS_TEST_TAILSCALE_BINARY;
    } else {
      process.env.THEIAOS_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
