...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: TheiaOSConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: TheiaOSConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: TheiaOSConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: TheiaOSConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: TheiaOSConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: TheiaOSConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: TheiaOSConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
