...
describe("gateway server models + voicewake", () => {
  const setTempHome = (homeDir: string) => {
    ...
    process.env.THEIAOS_STATE_DIR = path.join(homeDir, ".theiaos");
  ...
  test(
    ...
    { timeout: 60_000 },
    async () => {
      const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "theiaos-home-"));
      const restoreHome = setTempHome(homeDir);

      const initial = await rpcReq<{ triggers: string[] }>(ws, "voicewake.get");
      expect(initial.ok).toBe(true);
      expect(initial.payload?.triggers).toEqual(["theiaos", "claude", "computer"]);
      ...
      const onDisk = JSON.parse(
        await fs.readFile(path.join(homeDir, ".theiaos", "settings", "voicewake.json"), "utf8"),
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    const envSnapshot = captureEnv(["THEIAOS_CANVAS_HOST_PORT", "THEIAOS_GATEWAY_TOKEN"]);
    try {
      ...
      process.env.THEIAOS_CANVAS_HOST_PORT = String(canvasPort);
  ...
  test("auto-enables configured channel plugins on startup", async () => {
    const configPath = process.env.THEIAOS_CONFIG_PATH;
    if (!configPath) {
      throw new Error("Missing THEIAOS_CONFIG_PATH");
    }
