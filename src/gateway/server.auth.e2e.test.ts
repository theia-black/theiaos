...
describe("gateway server auth/connect", () => {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", { timeout: 60_000 }, async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.THEIAOS_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.THEIAOS_TEST_HANDSHAKE_TIMEOUT_MS = "50";
      try {
        const ws = await openWs(port);
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.THEIAOS_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.THEIAOS_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });
    ...
    test("connect (req) handshake prefers service version fallback in hello-ok payload", async () => {
      await withRuntimeVersionEnv(
        {
          THEIAOS_VERSION: " ",
          THEIAOS_SERVICE_VERSION: "2.4.6-service",
    ...
    test("connect (req) handshake prefers THEIAOS_VERSION over service version", async () => {
      await withRuntimeVersionEnv(
        {
          THEIAOS_VERSION: "9.9.9-cli",
          THEIAOS_SERVICE_VERSION: "2.4.6-service",
          npm_package_version: "1.0.0-package",
        },
    ...
    test("does not grant admin when scopes are omitted", async () => {
      ...
      const { identity, device } = await createSignedDevice({
        ...
        identityPath: path.join(os.tmpdir(), `theiaos-test-device-${randomUUID()}.json`),
      });
  ...
  describe("token auth", () => {
    ...
    beforeAll(async () => {
      prevToken = process.env.THEIAOS_GATEWAY_TOKEN;
      process.env.THEIAOS_GATEWAY_TOKEN = "secret";
      port = await getFreePort();
      server = await startGatewayServer(port);
    });
  ...
  test("requires pairing for scope upgrades", async () => {
    ...
    const { getPairedDevice } = await import("../infra/device-pairing.js");
    const { server, ws, port, prevToken } = await startServerWithClient("secret");
    const identityDir = await mkdtemp(join(tmpdir(), "theiaos-device-scope-"));
