...
export async function startGatewaySidecars(params: {
  ...
  pluginRegistry: ReturnType<typeof loadTheiaOSPlugins>;
  ...
  // Start TheiaOS browser control server (unless disabled via config).
  ...
  if (!isTruthyEnvValue(process.env.THEIAOS_SKIP_GMAIL_WATCHER)) {
    try {
  ...
  // Tests can opt out via THEIAOS_SKIP_CHANNELS (or legacy THEIAOS_SKIP_PROVIDERS).
  const skipChannels =
    isTruthyEnvValue(process.env.THEIAOS_SKIP_CHANNELS) ||
    isTruthyEnvValue(process.env.THEIAOS_SKIP_PROVIDERS);
  if (!skipChannels) {
    try {
    ...
    params.logChannels.info(
      "skipping channel start (THEIAOS_SKIP_CHANNELS=1 or THEIAOS_SKIP_PROVIDERS=1)",
    );
  }
