    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "theiaos", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-theiaos-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-theiaos-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "theiaos", messages: [{ role: "user", content: "hi" }] },
          {
            "x-theiaos-agent-id": "beta",
            "x-theiaos-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
