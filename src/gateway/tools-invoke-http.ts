...
import { createTheiaOSTools } from "../agents/theiaos-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-theiaos-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-theiaos-account-id")?.trim() || undefined;
  ...
  const allTools = createTheiaOSTools({
