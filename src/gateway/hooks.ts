...
export function resolveHooksConfig(cfg: TheiaOSConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: TheiaOSConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-theiaos-token"] === "string"
      ? req.headers["x-theiaos-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
