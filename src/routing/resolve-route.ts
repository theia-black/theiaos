...
function listAgents(cfg: TheiaOSConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: TheiaOSConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: TheiaOSConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<TheiaOSConfig, EvaluatedBindingsCache>();
