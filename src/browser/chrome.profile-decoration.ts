...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".theiaos-profile-decorated");
}
 ...
 */
export function decorateTheiaOSProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_THEIAOS_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_THEIAOS_BROWSER_COLOR).toUpperCase();
