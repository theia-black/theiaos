...
vi.mock("./chrome.js", () => ({
  resolveTheiaOSUserDataDir: vi.fn(() => "/tmp/theiaos-test/theiaos/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveTheiaOSUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "theiaos-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveTheiaOSUserDataDir).mockReturnValue(userDataDir);
