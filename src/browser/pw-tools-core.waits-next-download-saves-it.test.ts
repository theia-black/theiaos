...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredTheiaOSTmpDir: vi.fn(() => "/tmp/theiaos"),
}));
vi.mock("../infra/tmp-theiaos-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredTheiaOSTmpDir.mockReturnValue("/tmp/theiaos");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredTheiaOSTmpDir.mockReturnValue("/tmp/theiaos-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "theiaos-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredTheiaOSTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "theiaos-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "theiaos-preferred", "downloads")}${path.sep}`),
    );
  });
