...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-theiaos-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-theiaos-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-theiaos-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const operatorHint = isLocal
    ? `Restart the TheiaOS gateway (TheiaOS.app menubar, or \`${formatCliCommand("theiaos gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the TheiaOS browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the TheiaOS browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
