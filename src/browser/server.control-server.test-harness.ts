...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/theiaos" }));

beforeAll(async () => {
  chromeUserDataDir.dir = await fs.mkdtemp(path.join(os.tmpdir(), "theiaos-chrome-user-data-"));
});
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "theiaos",
        profiles: {
          theiaos: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchTheiaOSChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveTheiaOSUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopTheiaOSChrome: vi.fn(async () => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.THEIAOS_GATEWAY_PORT;
    process.env.THEIAOS_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.THEIAOS_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.THEIAOS_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayPort === undefined) {
      ...
      process.env.THEIAOS_GATEWAY_PORT = state.prevGatewayPort;
    }
    if (state.prevGatewayToken === undefined) {
      delete process.env.THEIAOS_GATEWAY_TOKEN;
    } else {
      process.env.THEIAOS_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.THEIAOS_GATEWAY_PASSWORD;
    } else {
      process.env.THEIAOS_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
