  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          theiaos: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.THEIAOS_GATEWAY_PORT;
    process.env.THEIAOS_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.THEIAOS_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.THEIAOS_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (prevGatewayPort === undefined) {
      ...
      process.env.THEIAOS_GATEWAY_PORT = prevGatewayPort;
    }
    if (prevGatewayToken === undefined) {
      delete process.env.THEIAOS_GATEWAY_TOKEN;
    } else {
      process.env.THEIAOS_GATEWAY_TOKEN = prevGatewayToken;
    }
    if (prevGatewayPassword === undefined) {
      delete process.env.THEIAOS_GATEWAY_PASSWORD;
    } else {
      process.env.THEIAOS_GATEWAY_PASSWORD = prevGatewayPassword;
    }
