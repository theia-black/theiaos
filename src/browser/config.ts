import type { BrowserConfig, BrowserProfileConfig, TheiaOSConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_THEIAOS_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "theiaos" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_THEIAOS_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "theiaos" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_THEIAOS_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_THEIAOS_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an TheiaOS browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for theiaos/theiaos CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: TheiaOSConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_THEIAOS_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "theiaos";
