...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under THEIAOS_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("theiaos-oauth-");
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to THEIAOS_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("theiaos-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
