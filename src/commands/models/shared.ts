...
export async function updateConfig(
  mutator: (cfg: TheiaOSConfig) => TheiaOSConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: TheiaOSConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: TheiaOSConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("theiaos agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
