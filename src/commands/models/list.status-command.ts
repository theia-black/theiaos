...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveTheiaOSAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("theiaos models auth setup-token")}\` or \`${formatCliCommand("theiaos configure")}\`.`
          : `Run \`${formatCliCommand("theiaos configure")}\` or set an API key env var.`;
