...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveTheiaOSAgentDir: vi.fn().mockReturnValue("/tmp/theiaos-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/theiaos-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/theiaos-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveTheiaOSAgentDir: mocks.resolveTheiaOSAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/theiaos-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveTheiaOSAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/theiaos-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/theiaos-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/theiaos-agent-custom");
