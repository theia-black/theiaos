...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "THEIAOS_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("THEIAOS_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv THEIAOS_GATEWAY_TOKEN");
    expect(message).not.toContain("THEIAOS_GATEWAY_PASSWORD");
  });
