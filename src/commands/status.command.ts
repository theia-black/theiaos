...
export async function statusCommand(
  ...
  runtime.log(theme.heading("TheiaOS status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("theiaos security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("theiaos security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.theiaos.ai/faq");
  runtime.log("Troubleshooting: https://docs.theiaos.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("theiaos status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("theiaos logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("theiaos status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("theiaos gateway probe")}`);
  }
}
