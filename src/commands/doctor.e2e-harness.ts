...
export const resolveTheiaOSPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadTheiaOSPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveTheiaOSPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.THEIAOS_STATE_DIR;
  originalUpdateInProgress = process.env.THEIAOS_UPDATE_IN_PROGRESS;
  process.env.THEIAOS_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "theiaos-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.THEIAOS_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.THEIAOS_UPDATE_IN_PROGRESS;
  } else {
    process.env.THEIAOS_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
