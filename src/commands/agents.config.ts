...
export function listAgentEntries(cfg: TheiaOSConfig): AgentEntry[] {
...
function resolveAgentName(cfg: TheiaOSConfig, agentId: string) {
...
function resolveAgentModel(cfg: TheiaOSConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: TheiaOSConfig): AgentSummary[] {
