...
const ensureTheiaOSModelsJson = vi.fn().mockResolvedValue(undefined);
const ensurePiAuthJsonFromAuthProfiles = vi
  .fn()
  .mockResolvedValue({ wrote: false, authPath: "/tmp/theiaos-agent/auth.json" });
const resolveTheiaOSAgentDir = vi.fn().mockReturnValue("/tmp/theiaos-agent");
...
vi.mock("../config/config.js", () => ({
  ...
  STATE_DIR: "/tmp/theiaos-state",
  loadConfig,
}));

vi.mock("../agents/models-config.js", () => ({
  ensureTheiaOSModelsJson,
}));
...
describe("models list/status", () => {
  ...
  it("models list syncs auth-profiles into auth.json before availability checks", async () => {
    ...
    expect(ensurePiAuthJsonFromAuthProfiles).toHaveBeenCalledWith("/tmp/theiaos-agent");
  });
