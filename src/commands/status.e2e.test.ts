...
beforeAll(() => {
  envSnapshot = captureEnv(["THEIAOS_PROFILE"]);
  process.env.THEIAOS_PROFILE = "isolated";
});
...
vi.mock("../infra/theiaos-root.js", () => ({
  resolveTheiaOSPackageRoot: vi.fn().mockResolvedValue("/tmp/theiaos"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/theiaos/pnpm-lock.yaml",
      markerPath: "/tmp/theiaos/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("TheiaOS status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("theiaos status --all") ||
          l.includes("theiaos --profile isolated status --all") ||
          l.includes("theiaos status --all") ||
          l.includes("theiaos --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.THEIAOS_GATEWAY_TOKEN = "abcd1234";
    try {
