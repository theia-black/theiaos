...
import type { TheiaOSConfig, GatewayBindMode } from "../config/config.js";
import { resolveGatewayAuth } from "../gateway/auth.js";
import { isLoopbackHost, resolveGatewayBindHost } from "../gateway/net.js";
import { readChannelAllowFromStore } from "../pairing/pairing-store.js";
import { note } from "../terminal/note.js";

export async function noteSecurityWarnings(cfg: TheiaOSConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("theiaos security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("theiaos configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("theiaos config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("theiaos doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("theiaos config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('theiaos config set session.dmScope "per-channel-peer"') +
