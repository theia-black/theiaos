...
function resolveMode(cfg: TheiaOSConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("TheiaOS doctor");

  const root = await resolveTheiaOSPackageRoot({
  ...
  let cfg: TheiaOSConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("theiaos configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("theiaos config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("theiaos setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("theiaos doctor --fix")}" to apply changes.`);
  }
