...
const SEARCH_TOOL = "https://docs.theiaos.ai/mcp.SearchTheiaOS";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.theiaos.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('theiaos docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.theiaos.ai/");
      runtime.log(`Search: ${formatCliCommand('theiaos docs "your query"')}`);
    }
    return;
  }
