...
describe("parseRelaySmokeTest", () => {
  ...
  it("parses env var smoke mode only when no args", () => {
    expect(parseRelaySmokeTest([], { THEIAOS_SMOKE_QR: "1" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { THEIAOS_SMOKE_QR: "1" })).toBe(null);
  });

  it("supports THEIAOS_SMOKE=qr only when no args", () => {
    expect(parseRelaySmokeTest([], { THEIAOS_SMOKE: "qr" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { THEIAOS_SMOKE: "qr" })).toBe(null);
  });
  ...
  it("prefers explicit --smoke over env vars", () => {
    expect(parseRelaySmokeTest(["--smoke", "qr"], { THEIAOS_SMOKE: "nope" })).toBe("qr");
  });
});
