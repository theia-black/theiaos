...
declare const __THEIAOS_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __THEIAOS_VERSION__ === "string" && __THEIAOS_VERSION__) ||
  process.env.THEIAOS_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `theiaos --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.THEIAOS_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.THEIAOS_GATEWAY_BIND ??
        ...
        if (isRestart) {
          ...
          if (respawn.mode === "spawned" || respawn.mode === "supervised") {
            ...
            if (respawn.mode === "failed") {
              ...
              defaultRuntime.log("gateway: restart mode in-process restart (THEIAOS_NO_RESPAWN)");
            }
            shuttingDown = false;
            restartResolver?.();
          }
