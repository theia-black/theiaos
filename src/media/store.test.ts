...
describe("media store", () => {
  ...
  beforeAll(async () => {
    ...
    home = await fs.mkdtemp(path.join(os.tmpdir(), "theiaos-test-home-"));
    ...
    await fs.mkdir(path.join(home, ".theiaos"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.theiaos${path.sep}media`);
