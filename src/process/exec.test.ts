...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    const envSnapshot = captureEnv(["THEIAOS_BASE_ENV"]);
    process.env.THEIAOS_BASE_ENV = "base";
    try {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.THEIAOS_BASE_ENV ?? "") + "|" + (process.env.THEIAOS_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { THEIAOS_TEST_ENV: "ok" },
        },
      );
