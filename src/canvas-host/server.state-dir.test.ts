...
describe("canvas host state dir defaults", () => {
  ...
  it("uses THEIAOS_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "theiaos-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("TheiaOS Canvas");
    } finally {
      await handler.close();
