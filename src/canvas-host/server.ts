...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.theiaosCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.theiaosCanvasA2UIAction &&
        typeof window.theiaosCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.theiaosSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("theiaos:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android TheiaOS node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.theiaosSendUserAction === "function"
        ? window.theiaosSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.THEIAOS_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.THEIAOS_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>TheiaOS Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
