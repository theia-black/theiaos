...
import type { TheiaOSConfig, ConfigFileSnapshot } from "../config/types.theiaos.js";
...
vi.mock("../infra/theiaos-root.js", () => ({
  resolveTheiaOSPackageRoot: vi.fn(),
}));
...
const { resolveTheiaOSPackageRoot } = await import("../infra/theiaos-root.js");
...
describe("update-cli", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "theiaos-update-tests-"));
  });
  ...
  const baseConfig = {} as TheiaOSConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/theiaos-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = await createCaseDir("theiaos-update");
    setTty(false);
    readPackageVersion.mockResolvedValue("2.0.0");

    vi.mocked(resolveTheiaOSPackageRoot).mockResolvedValue(tempDir);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveTheiaOSPackageRoot).mockReset();
    ...
    vi.mocked(resolveTheiaOSPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("theiaos");
    readPackageVersion.mockResolvedValue("1.0.0");
    resolveGlobalManager.mockResolvedValue("npm");
    serviceLoaded.mockResolvedValue(false);
    prepareRestartScript.mockResolvedValue("/tmp/theiaos-restart-test.sh");
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("TheiaOS update status");
  });
  ...
  it("uses stored beta channel when configured", async () => {
    vi.mocked(readConfigFileSnapshot).mockResolvedValue({
      ...baseSnapshot,
      config: { update: { channel: "beta" } } as TheiaOSConfig,
    });
    vi.mocked(runGatewayUpdate).mockResolvedValue({
      status: "ok",
      mode: "git",
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    const envSnapshot = captureEnv(["THEIAOS_UPDATE_IN_PROGRESS"]);
    ...
    try {
      ...
      expect(process.env.THEIAOS_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await createCaseDir("theiaos-update-wizard");
    const envSnapshot = captureEnv(["THEIAOS_GIT_DIR"]);
    try {
      setTty(true);
      process.env.THEIAOS_GIT_DIR = tempDir;
