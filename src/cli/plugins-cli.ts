...
function applySlotSelectionForPlugin(
  ...
  pluginId: string,
): { config: TheiaOSConfig; warnings: string[] } {
...
function enablePluginInConfig(config: TheiaOSConfig, pluginId: string): TheiaOSConfig {
  return {
    ...config,
    plugins: {
      ...config.plugins,
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage TheiaOS plugins and extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.theiaos.ai/cli/plugins")}\n`,
    );
    ...
    .action(async (raw: string, opts: { link?: boolean }) => {
      ...
      if (fs.existsSync(resolved)) {
        if (opts.link) {
          ...
          let next: TheiaOSConfig = enablePluginInConfig(
            {
              ...cfg,
              plugins: {
                ...cfg.plugins,
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.theiaos.ai/plugin");
      lines.push("");
