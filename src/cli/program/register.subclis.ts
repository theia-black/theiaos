...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.THEIAOS_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.THEIAOS_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<TheiaOSConfig> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live TheiaOS docs",
    hasSubcommands: false,
    register: async (program) => {
