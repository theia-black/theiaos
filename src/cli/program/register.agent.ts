...
${formatHelpExamples([
  ['theiaos agent --to +15555550123 --message "status update"', "Start a new session."],
  ['theiaos agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'theiaos agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'theiaos agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['theiaos agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.theiaos.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.theiaos.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "theiaos agents set-identity --identity-file ~/.theiaos/workspace/IDENTITY.md --agent main",
