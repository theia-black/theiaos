...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    THEIAOS_STATE_DIR: process.env.THEIAOS_STATE_DIR,
    THEIAOS_CONFIG_PATH: process.env.THEIAOS_CONFIG_PATH,
    THEIAOS_GATEWAY_PORT: process.env.THEIAOS_GATEWAY_PORT,
    THEIAOS_PROFILE: process.env.THEIAOS_PROFILE,
  };

  beforeEach(() => {
    process.env.THEIAOS_STATE_DIR = "/tmp/theiaos-cli-state";
    process.env.THEIAOS_CONFIG_PATH = "/tmp/theiaos-cli-state/theiaos.json";
    delete process.env.THEIAOS_GATEWAY_PORT;
    delete process.env.THEIAOS_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.THEIAOS_STATE_DIR !== undefined) {
      process.env.THEIAOS_STATE_DIR = originalEnv.THEIAOS_STATE_DIR;
    } else {
      delete process.env.THEIAOS_STATE_DIR;
    }

    if (originalEnv.THEIAOS_CONFIG_PATH !== undefined) {
      process.env.THEIAOS_CONFIG_PATH = originalEnv.THEIAOS_CONFIG_PATH;
    } else {
      delete process.env.THEIAOS_CONFIG_PATH;
    }

    if (originalEnv.THEIAOS_GATEWAY_PORT !== undefined) {
      process.env.THEIAOS_GATEWAY_PORT = originalEnv.THEIAOS_GATEWAY_PORT;
    } else {
      delete process.env.THEIAOS_GATEWAY_PORT;
    }

    if (originalEnv.THEIAOS_PROFILE !== undefined) {
      process.env.THEIAOS_PROFILE = originalEnv.THEIAOS_PROFILE;
    } else {
      delete process.env.THEIAOS_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        THEIAOS_PROFILE: "dev",
        THEIAOS_STATE_DIR: "/tmp/theiaos-daemon-state",
        THEIAOS_CONFIG_PATH: "/tmp/theiaos-daemon-state/theiaos.json",
        THEIAOS_GATEWAY_PORT: "19001",
      },
