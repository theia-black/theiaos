...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "theiaos-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      THEIAOS_NO_RESPAWN: "1",
      THEIAOS_STATE_DIR: stateDir,
      THEIAOS_SKIP_CHANNELS: "1",
      THEIAOS_SKIP_GMAIL_WATCHER: "1",
      THEIAOS_SKIP_CRON: "1",
      THEIAOS_SKIP_BROWSER_CONTROL_SERVER: "1",
      THEIAOS_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "theiaos-entry-bootstrap.mjs");
