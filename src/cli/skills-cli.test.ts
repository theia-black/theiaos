...
describe("skills-cli", () => {
  ...
  describe("integration: loads real skills from bundled directory", () => {
    ...
    beforeAll(async () => {
      envSnapshot = captureEnv(["THEIAOS_BUNDLED_SKILLS_DIR"]);
      tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "theiaos-skills-test-"));
      tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "theiaos-bundled-skills-test-"));
      process.env.THEIAOS_BUNDLED_SKILLS_DIR = tempBundledDir;
