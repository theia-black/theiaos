    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["theiaos memory status", "Show index and provider status."],
          ["theiaos memory index --force", "Force a full reindex."],
          ['theiaos memory search --query "deployment notes"', "Search indexed memory entries."],
          ["theiaos memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.theiaos.ai/cli/memory")}\n`,
    );
