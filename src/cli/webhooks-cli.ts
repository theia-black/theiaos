...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.theiaos.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + TheiaOS hooks")
    ...
    .option("--hook-url <url>", "TheiaOS hook URL")
    .option("--hook-token <token>", "TheiaOS hook token")
