    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["theiaos nodes status", "List known nodes with live status."],
          ["theiaos nodes pairing pending", "Show pending node pairing requests."],
          ['theiaos nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["theiaos nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.theiaos.ai/cli/nodes")}\n`,
    );
