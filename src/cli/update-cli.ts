...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update TheiaOS and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["theiaos update", "Update a source checkout (git)"],
        ["theiaos update --channel beta", "Switch to beta channel (git + npm)"],
        ["theiaos update --channel dev", "Switch to dev channel (git + npm)"],
        ["theiaos update --tag beta", "One-off update to a dist-tag or version"],
        ["theiaos update --no-restart", "Update without restarting the service"],
        ["theiaos update --json", "Output result as JSON"],
        ["theiaos update --yes", "Non-interactive (accept downgrade prompts)"],
        ["theiaos update wizard", "Interactive update wizard"],
        ["theiaos --update", "Shorthand for theiaos update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run theiaos update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.theiaos.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["theiaos update status", "Show channel + version status."],
          ["theiaos update status --json", "JSON output."],
          ["theiaos update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.theiaos.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
