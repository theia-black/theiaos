...
import { resolveTheiaOSPackageRoot } from "../../infra/theiaos-root.js";
...
const THEIAOS_REPO_URL = "https://github.com/theia-black/theiaos.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "theiaos";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("theiaos@")) {
    return trimmed.slice("theiaos@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.THEIAOS_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveTheiaOSPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", THEIAOS_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `THEIAOS_GIT_DIR points at a non-git directory: ${params.dir}. Set THEIAOS_GIT_DIR to an empty folder or an theiaos checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`THEIAOS_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "theiaos.mjs");
