...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("theiaos node install"), formatCliCommand("theiaos node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("theiaos node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    THEIAOS_LOG_PREFIX: baseEnv.THEIAOS_LOG_PREFIX ?? "node",
