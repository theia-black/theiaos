    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "theiaos node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["theiaos node status", "Check node host service status."],
          ["theiaos node install", "Install the node host service."],
          ["theiaos node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.theiaos.ai/cli/node")}\n`,
    );
