...
import { discoverTheiaOSPlugins } from "../../plugins/discovery.js";
import type { TheiaOSPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, TheiaOSPackageManifest>>;
...
const ENV_CATALOG_PATHS = ["THEIAOS_PLUGIN_CATALOG_PATHS", "THEIAOS_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<TheiaOSPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: TheiaOSPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: TheiaOSPackageManifest;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverTheiaOSPlugins({ workspaceDir: options.workspaceDir });
