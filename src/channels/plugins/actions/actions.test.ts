...
function telegramCfg(): TheiaOSConfig {
  return { channels: { telegram: { botToken: "tok" } } } as TheiaOSConfig;
}

function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as TheiaOSConfig;
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as TheiaOSConfig;
