...
function cfg(accounts?: Record<string, unknown> | null): TheiaOSConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as TheiaOSConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as TheiaOSConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as TheiaOSConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as TheiaOSConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as TheiaOSConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as TheiaOSConfig)).toBe("default");
    });
  });
});
