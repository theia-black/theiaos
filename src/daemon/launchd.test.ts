...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.theiaos.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", THEIAOS_PROFILE: "default" },
    });
...
describe("launchd bootstrap repair", () => {
  it("bootstraps and kickstarts the resolved label", async () => {
    ...
    const label = "ai.theiaos.gateway";
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when THEIAOS_PROFILE is unset", () => {
    const env = { HOME: "/Users/test" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.theiaos.gateway.plist",
    );
  });

  it("uses profile-specific label when THEIAOS_PROFILE is set to a custom value", () => {
    const env = { HOME: "/Users/test", THEIAOS_PROFILE: "jbphoenix" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.theiaos.jbphoenix.plist",
    );
  });

  it("prefers THEIAOS_LAUNCHD_LABEL over THEIAOS_PROFILE", () => {
    const env = {
      HOME: "/Users/test",
      THEIAOS_PROFILE: "jbphoenix",
      THEIAOS_LAUNCHD_LABEL: "com.custom.label",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
  ...
  it("trims whitespace from THEIAOS_LAUNCHD_LABEL", () => {
    const env = {
      HOME: "/Users/test",
      THEIAOS_LAUNCHD_LABEL: "  com.custom.label  ",
    };
  ...
  it("ignores empty THEIAOS_LAUNCHD_LABEL and falls back to profile", () => {
    const env = {
      HOME: "/Users/test",
      THEIAOS_PROFILE: "myprofile",
      THEIAOS_LAUNCHD_LABEL: "   ",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.theiaos.myprofile.plist",
    );
  });
});
