...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.THEIAOS_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service THEIAOS_GATEWAY_TOKEN does not match gateway.auth.token in theiaos.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `theiaos gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
