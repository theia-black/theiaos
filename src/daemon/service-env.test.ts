...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.THEIAOS_GATEWAY_PORT).toBe("18789");
    expect(env.THEIAOS_GATEWAY_TOKEN).toBe("secret");
    expect(env.THEIAOS_SERVICE_MARKER).toBe("theiaos");
    expect(env.THEIAOS_SERVICE_KIND).toBe("gateway");
    expect(typeof env.THEIAOS_SERVICE_VERSION).toBe("string");
    expect(env.THEIAOS_SYSTEMD_UNIT).toBe("theiaos-gateway.service");
    if (process.platform === "darwin") {
      expect(env.THEIAOS_LAUNCHD_LABEL).toBe("ai.theiaos.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", THEIAOS_PROFILE: "work" },
      port: 18789,
    });
    expect(env.THEIAOS_SYSTEMD_UNIT).toBe("theiaos-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.THEIAOS_LAUNCHD_LABEL).toBe("ai.theiaos.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".theiaos"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", THEIAOS_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".theiaos-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", THEIAOS_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".theiaos"));
  });

  it("uses THEIAOS_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", THEIAOS_STATE_DIR: "/var/lib/theiaos" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/theiaos"));
  });

  it("expands ~ in THEIAOS_STATE_DIR", () => {
    const env = { HOME: "/Users/test", THEIAOS_STATE_DIR: "~/theiaos-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/theiaos-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { THEIAOS_STATE_DIR: "C:\\State\\theiaos" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\theiaos");
  });
});
