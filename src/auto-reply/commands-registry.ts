...
export function isCommandEnabled(cfg: TheiaOSConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: TheiaOSConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as TheiaOSConfig),
...
export function getCommandDetection(_cfg?: TheiaOSConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: TheiaOSConfig) {
