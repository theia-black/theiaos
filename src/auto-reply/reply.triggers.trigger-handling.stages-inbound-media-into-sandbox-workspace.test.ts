...
describe("stageSandboxMedia", () => {
  it("stages inbound media into the sandbox workspace", async () => {
    await withSandboxMediaTempHome("theiaos-triggers-", async (home) => {
      const inboundDir = join(home, ".theiaos", "media", "inbound");
      ...
      await stageSandboxMedia({
        ...
        workspaceDir: join(home, "theiaos"),
      });
  ...
  it("rejects staging host files from outside the media directory", async () => {
    await withSandboxMediaTempHome("theiaos-triggers-bypass-", async (home) => {
      // Sensitive host file outside .theiaos
