...
import { scheduleGatewaySigusr1Restart, triggerTheiaOSRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting TheiaOS in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerTheiaOSRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting TheiaOS via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
