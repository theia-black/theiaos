...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  function snapshotStateEnv(): StateEnvSnapshot {
    return { THEIAOS_STATE_DIR: process.env.THEIAOS_STATE_DIR };
  }

  function restoreStateEnv(snapshot: StateEnvSnapshot) {
    if (snapshot.THEIAOS_STATE_DIR === undefined) {
      delete process.env.THEIAOS_STATE_DIR;
    } else {
      process.env.THEIAOS_STATE_DIR = snapshot.THEIAOS_STATE_DIR;
    }
  }
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "theiaos-typing-heartbeat-"));
  });
...
describe("runReplyAgent memory flush", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "theiaos-memory-flush-"));
  });
