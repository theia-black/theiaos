...
function createReplyConfig(home: string, streamMode?: "block"): TheiaOSConfig {
  return {
    agents: {
      defaults: {
...
describe("block streaming", () => {
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "theiaos-stream-"));
  });
  ...
  beforeEach(() => {
    vi.stubEnv("THEIAOS_TEST_FAST", "1");
