...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "theiaos-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "theiaos-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "theiaos-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "theiaos-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.theiaos when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "theiaos-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers THEIAOS_HOME over HOME", () => {
    vi.stubEnv("THEIAOS_HOME", "/srv/theiaos-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/theiaos-home"));
...
describe("shortenHomePath", () => {
  it("uses $THEIAOS_HOME prefix when THEIAOS_HOME is set", () => {
    vi.stubEnv("THEIAOS_HOME", "/srv/theiaos-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/theiaos-home")}/.theiaos/theiaos.json`)).toBe(
      "$THEIAOS_HOME/.theiaos/theiaos.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $THEIAOS_HOME replacement when THEIAOS_HOME is set", () => {
    vi.stubEnv("THEIAOS_HOME", "/srv/theiaos-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/theiaos-home")}/.theiaos/theiaos.json`),
    ).toBe("config: $THEIAOS_HOME/.theiaos/theiaos.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/theiaos")).toBe(path.resolve(os.homedir(), "theiaos"));
  });
  ...
  it("prefers THEIAOS_HOME for tilde expansion", () => {
    vi.stubEnv("THEIAOS_HOME", "/srv/theiaos-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/theiaos")).toBe(path.resolve("/srv/theiaos-home", "theiaos"));
