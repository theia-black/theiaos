...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "theiaos-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    originalBundledDir = process.env.THEIAOS_BUNDLED_HOOKS_DIR;
    process.env.THEIAOS_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(async () => {
    ...
    if (originalBundledDir === undefined) {
      delete process.env.THEIAOS_BUNDLED_HOOKS_DIR;
    } else {
      process.env.THEIAOS_BUNDLED_HOOKS_DIR = originalBundledDir;
    }
  });
