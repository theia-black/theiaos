...
This directory contains hooks that ship with TheiaOS. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.theiaos/theiaos.json`:
...
2. Restart gateway: `pkill -9 -f 'theiaos.*gateway' && pnpm theiaos gateway`
3. Enable the hook: `theiaos hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.theiaos.ai/automation/hooks
