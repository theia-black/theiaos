...
homepage: https://docs.theiaos.ai/automation/hooks#command-logger
metadata:
  {
    "theiaos":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.theiaos/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.theiaos/logs/commands.log ~/.theiaos/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/theiaos`:
   ```
   /home/username/.theiaos/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
