...
`;
    ...
    expect(parsed.theiaos.emoji).toBe("💾");
    expect(parsed.theiaos.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.theiaos.emoji).toBe("📝");
    expect(parsed.theiaos.events).toEqual(["command"]);
    expect(parsed.theiaos.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.theiaos.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"theiaos": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"theiaos": {"events": ["test"]}}');
  });
...
describe("resolveTheiaOSMetadata", () => {
  it("extracts theiaos metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveTheiaOSMetadata(frontmatter);
  ...
  it("returns undefined when theiaos key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        theiaos: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with TheiaOS" },
            { id: "npm", kind: "npm", package: "@theiaos/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@theiaos/hook");
  });
    ...
    "theiaos":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with TheiaOS" }],
      },
  }
---
...
`;
    ...
    const theiaos = resolveTheiaOSMetadata(frontmatter);
    expect(theiaos).toBeDefined();
    expect(theiaos?.emoji).toBe("💾");
    expect(theiaos?.events).toEqual(["command:new"]);
    expect(theiaos?.requires?.config).toEqual(["workspace.dir"]);
    expect(theiaos?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const theiaos = resolveTheiaOSMetadata(frontmatter);
    expect(theiaos?.emoji).toBe("disk");
    expect(theiaos?.events).toEqual(["command:new"]);
  });
});
