...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: TheiaOSPluginDefinition;
  register?: TheiaOSPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as TheiaOSPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as TheiaOSPluginDefinition;
...
export function loadTheiaOSPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverTheiaOSPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "theiaos/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "theiaos/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: TheiaOSPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as TheiaOSPluginModule;
    } catch (err) {
