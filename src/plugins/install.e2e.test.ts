...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `theiaos-plugin-install-${randomUUID()}`);
    ...
    JSON.stringify(
      {
        ...
        theiaos: { extensions: params.extensions },
      },
      null,
      2,
    ),
...
async function expectArchiveInstallReservedSegmentRejection(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      version: "0.0.1",
      theiaos: { extensions: ["./dist/index.js"] },
    }),
    "utf-8",
  );
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.theiaos/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without theiaos.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@theiaos/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    expect(result.error).toContain("theiaos.extensions");
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(argv).toEqual(["npm", "pack", "@theiaos/voice-call@0.0.1", "--ignore-scripts"]);
