...
function createSinglePluginWithEmptySlotsConfig(): TheiaOSConfig {
  return {
    plugins: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "theiaos-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        source: "npm",
        spec: "my-plugin@1.0.0",
        installPath: "/tmp/not-theiaos-extensions/my-plugin",
      },
      extensionsDir,
    });
