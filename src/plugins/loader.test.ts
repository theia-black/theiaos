...
import { loadTheiaOSPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `theiaos-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.THEIAOS_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "theiaos.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.THEIAOS_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadTheiaOSPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    ...
    process.env.THEIAOS_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadTheiaOSPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadTheiaOSPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadTheiaOSPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
