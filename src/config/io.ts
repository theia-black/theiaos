...
import type { TheiaOSConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareTheiaOSVersions } from "./version.js";
...
function coerceConfig(value: unknown): TheiaOSConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as TheiaOSConfig;
}
...
function stampConfigVersion(cfg: TheiaOSConfig): TheiaOSConfig {
...
function warnIfConfigFromFuture(cfg: TheiaOSConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareTheiaOSVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer TheiaOS (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as TheiaOSConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): TheiaOSConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as TheiaOSConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: TheiaOSConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as TheiaOSConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.THEIAOS_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.THEIAOS_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.THEIAOS_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.THEIAOS_WATCH_SESSION === "string" &&
        deps.env.THEIAOS_WATCH_SESSION.trim().length > 0
          ? deps.env.THEIAOS_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.THEIAOS_WATCH_COMMAND === "string" &&
        deps.env.THEIAOS_WATCH_COMMAND.trim().length > 0
          ? deps.env.THEIAOS_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `THEIAOS_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.THEIAOS_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.THEIAOS_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): TheiaOSConfig {
