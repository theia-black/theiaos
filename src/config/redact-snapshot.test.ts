...
import type { ConfigFileSnapshot } from "./types.theiaos.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.theiaos/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/theiaos/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/theiaos/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/theiaos/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/theiaos/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = TheiaOSSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "TheiaOSConfig";
    const hints = mapSensitivePaths(TheiaOSSchema, "", {});
