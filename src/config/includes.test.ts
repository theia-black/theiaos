...
const ETC_THEIAOS_DIR = path.join(ROOT_DIR, "etc", "theiaos");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "theiaos.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcTheiaOSPath(...parts: string[]) {
  return path.join(ETC_THEIAOS_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("rejects absolute path outside config directory (CWE-22)", () => {
    const absolute = etcTheiaOSPath("agents.json");
  ...
  it("rejects parent directory traversal escaping config directory (CWE-22)", () => {
    ...
    expect(() => resolve(obj, files, configPath("sub", "theiaos.json"))).toThrow(
      ConfigIncludeError,
    );
    expect(() => resolve(obj, files, configPath("sub", "theiaos.json"))).toThrow(
...
describe("security: path traversal protection (CWE-22)", () => {
  ...
  describe("edge cases", () => {
    ...
    it("allows include files when the config root path is a symlink", async () => {
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "theiaos-includes-symlink-"));
      try {
        ...
        const result = resolveConfigIncludes(
          ...
          path.join(linkRoot, "theiaos.json"),
        );
