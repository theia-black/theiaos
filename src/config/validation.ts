...
import type { TheiaOSConfig, ConfigValidationIssue } from "./types.js";
import { TheiaOSSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: TheiaOSConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: TheiaOSConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = TheiaOSSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as TheiaOSConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as TheiaOSConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as TheiaOSConfig,
  };
}
