...
import type { TheiaOSConfig } from "../config/types.js";
import { resolvePreferredTheiaOSTmpDir } from "../infra/tmp-theiaos-dir.js";
import { readLoggingConfig } from "./config.js";
import type { ConsoleStyle } from "./console.js";
import { type LogLevel, levelToMinLevel, normalizeLogLevel } from "./levels.js";
import { loggingState } from "./state.js";

export const DEFAULT_LOG_DIR = resolvePreferredTheiaOSTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "theiaos.log"); // legacy single-file path

const LOG_PREFIX = "theiaos";
...
function resolveSettings(): ResolvedSettings {
  let cfg: TheiaOSConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig("../config/config.js") as {
        loadConfig?: () => TheiaOSConfig;
      };
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.THEIAOS_TEST_FILE_LOG !== "1" ? "silent" : "info";
