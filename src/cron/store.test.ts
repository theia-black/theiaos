...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "theiaos-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses THEIAOS_HOME for tilde expansion", () => {
    vi.stubEnv("THEIAOS_HOME", "/srv/theiaos-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/theiaos-home", "cron", "jobs.json"));
  });
});
