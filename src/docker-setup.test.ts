...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "theiaos-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    THEIAOS_GATEWAY_TOKEN: "test-token",
    THEIAOS_CONFIG_DIR: join(sandbox.rootDir, "config"),
    THEIAOS_WORKSPACE_DIR: join(sandbox.rootDir, "theiaos"),
  };
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      ...
      env: createEnv(sandbox, {
        THEIAOS_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
        THEIAOS_EXTRA_MOUNTS: undefined,
        THEIAOS_HOME_VOLUME: "theiaos-home",
      }),
      stdio: ["ignore", "ignore", "pipe"],
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("THEIAOS_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("THEIAOS_EXTRA_MOUNTS=");
    expect(envFile).toContain("THEIAOS_HOME_VOLUME=theiaos-home");
    const extraCompose = await readFile(join(sandbox.rootDir, "docker-compose.extra.yml"), "utf8");
    expect(extraCompose).toContain("theiaos-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("theiaos-home:");
    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg THEIAOS_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
