...
ZIP=${1:?"Usage: $0 TheiaOS-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/theiaos/theiaos/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^TheiaOS-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/theia-black/theiaos/releases/download/v${VERSION}/"}
