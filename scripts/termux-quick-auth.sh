...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/theiaos/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "TheiaOS Re-Auth" -c "After getting key, run: ssh $SERVER '~/theiaos/scripts/mobile-reauth.sh'" --id theiaos-auth
        ;;
    *)
