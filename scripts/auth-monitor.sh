...
#
# Suggested cron: */30 * * * * /home/admin/theiaos/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send TheiaOS notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., theiaos-alerts)
...
STATE_FILE="$HOME/.theiaos/auth-monitor-state"
...
send_notification() {
    ...
    # Send via TheiaOS if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use theiaos
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via TheiaOS to $NOTIFY_PHONE..."
            theiaos send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! TheiaOS is down. Run: ssh l36 '~/theiaos/scripts/mobile-reauth.sh'" "urgent"
    exit 1
