...
show_usage() {
    cat << EOF
clawlog - TheiaOS Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View TheiaOS logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    TheiaOS logs flow through the macOS unified log (subsystem: ai.theiaos).
