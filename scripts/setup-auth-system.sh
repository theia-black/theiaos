#!/bin/bash
# Setup TheiaOS Auth Management System
...
echo "=== TheiaOS Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'theiaos-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  TheiaOS message: Send warning via TheiaOS itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/theiaos-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/theiaos-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/theiaos-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now theiaos-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status theiaos-auth-monitor.timer"
echo ""
echo "Quick commands:"
