...
const shouldBuild = (deps) => {
  if (deps.env.THEIAOS_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.THEIAOS_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[theiaos] ${message}\n`);
};

const runTheiaOS = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["theiaos.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
