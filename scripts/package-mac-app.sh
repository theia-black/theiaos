...
# Build and bundle TheiaOS into a minimal .app we can open.
# Outputs to dist/TheiaOS.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/TheiaOS.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="TheiaOS"
BUNDLE_ID="${BUNDLE_ID:-ai.theiaos.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/theiaos/theiaos/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/TheiaOS/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :TheiaOSBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :TheiaOSGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/TheiaOS"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/TheiaOS"
fi
chmod +x "$APP_ROOT/Contents/MacOS/TheiaOS"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/TheiaOS" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/TheiaOS/Resources/TheiaOS.icns" "$APP_ROOT/Contents/Resources/TheiaOS.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/TheiaOS/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying TheiaOSKit resources"
THEIAOSKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/TheiaOSKit_TheiaOSKit.bundle"
if [ -d "$THEIAOSKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/TheiaOSKit_TheiaOSKit.bundle"
  cp -R "$THEIAOSKIT_BUNDLE" "$APP_ROOT/Contents/Resources/TheiaOSKit_TheiaOSKit.bundle"
else
  echo "WARN: TheiaOSKit resource bundle not found at $THEIAOSKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running TheiaOS"
killall -q TheiaOS 2>/dev/null || true
