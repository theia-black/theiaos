...
INSTALL_URL="${THEIAOS_INSTALL_URL:-https://theiaos.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${THEIAOS_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${THEIAOS_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="theiaos"
PACKAGE_NAME="${THEIAOS_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (THEIAOS_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${THEIAOS_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${THEIAOS_INSTALL_LATEST_OUT:-}"
fi
