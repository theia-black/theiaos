...
IMAGE_NAME="theiaos-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="theiaos-net-e2e-$$"
GW_NAME="theiaos-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "THEIAOS_GATEWAY_TOKEN=$TOKEN" \
	  -e "THEIAOS_SKIP_CHANNELS=1" \
	  -e "THEIAOS_SKIP_GMAIL_WATCHER=1" \
	  -e "THEIAOS_SKIP_CRON=1" \
	  -e "THEIAOS_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
