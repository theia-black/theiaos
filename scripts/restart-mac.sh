#!/usr/bin/env bash
# Reset TheiaOS like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${THEIAOS_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="TheiaOS.app/Contents/MacOS/TheiaOS"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/TheiaOS"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/TheiaOS"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/TheiaOS"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.theiaos.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/theiaos-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${THEIAOS_RESTART_LOG:-/tmp/theiaos-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${THEIAOS_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.theiaos/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  THEIAOS_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node theiaos.mjs daemon install --force --runtime node"
      log "  node theiaos.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.theiaos/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.theiaos.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing TheiaOS instances"
kill_all_theiaos
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product TheiaOS"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/TheiaOS.app" ]]; then
    APP_BUNDLE="/Applications/TheiaOS.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/TheiaOS.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/TheiaOS.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/TheiaOS.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set THEIAOS_APP_BUNDLE to your installed TheiaOS.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node theiaos.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node theiaos.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".theiaos", "theiaos.json"), "utf8");
