 ...
 * `theiaos update`. Use it to verify the completion installation flow
...
\`theiaos update\`, \`theiaos doctor\`, and \`theiaos onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# TheiaOS Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`theiaos completion --install\` later to enable.`));
    return;
  }
