#!/usr/bin/env bash
# ClawDock - Docker helpers for TheiaOS
# Inspired by Simon Willison's "Running TheiaOS in Docker"
# https://til.simonwillison.net/llms/theiaos-docker
#
# Installation:
...
# Common paths to check for TheiaOS
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/theiaos"
  "${HOME}/workspace/theiaos"
  "${HOME}/projects/theiaos"
  "${HOME}/dev/theiaos"
  "${HOME}/code/theiaos"
  "${HOME}/src/theiaos"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found TheiaOS at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/theiaos"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ TheiaOS not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/theia-black/theiaos.git ~/theiaos"
    echo "  cd ~/theiaos && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^THEIAOS_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d theiaos-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart theiaos-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f theiaos-gateway
}
...
clawdock-workspace() {
  cd ~/.theiaos/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec theiaos-gateway \
    bash -c 'echo "alias theiaos=\"./theiaos.mjs\"" > /tmp/.bashrc_theiaos && bash --rcfile /tmp/.bashrc_theiaos'
}
...
clawdock-cli() {
  _clawdock_compose run --rm theiaos-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build theiaos-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "THEIAOS_GATEWAY_TOKEN=$token" theiaos-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" theiaos-gateway \
    bash -c './theiaos.mjs config set gateway.remote.token "$TOKEN" && ./theiaos.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec theiaos-gateway \
    bash -c "./theiaos.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart theiaos-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm theiaos-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec theiaos-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'theiaos config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for TheiaOS${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (theiaos alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to theiaos project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.theiaos)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'theiaos channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'theiaos status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.theiaos.ai${_CLR_RESET}"
  echo ""
}
