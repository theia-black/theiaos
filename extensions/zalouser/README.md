...
TheiaOS extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
theiaos channels login --channel zalouser
...
```bash
theiaos message send --channel zalouser --target <threadId> --message "Hello from TheiaOS!"
```
...
```bash
theiaos channels login --channel zalouser              # Login via QR
theiaos channels login --channel zalouser --account work
theiaos channels status --probe
theiaos channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
theiaos directory self --channel zalouser
theiaos directory peers list --channel zalouser --query "name"
theiaos directory groups list --channel zalouser --query "work"
theiaos directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
theiaos message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
theiaos message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `theiaos channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
theiaos channels login --channel zalouser --account work
theiaos message send --channel zalouser --account work --target <id> --message "Hello"
