...
import type { TheiaOSPluginApi, TheiaOSPluginToolContext } from "../../../src/plugins/types.js";
...
function fakeApi(overrides: Partial<TheiaOSPluginApi> = {}): TheiaOSPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<TheiaOSPluginToolContext> = {}): TheiaOSPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "theiaos-lobster-plugin-"));
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: TheiaOSPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
