...
function resolveTheiaOSRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.THEIAOS_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set THEIAOS_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveTheiaOSRoot(), "dist", "extensionAPI.js");
