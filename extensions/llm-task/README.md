...
`theiaos.invoke --each`) without adding new TheiaOS code per workflow.
...
This extension depends on TheiaOS internal modules (the embedded agent runner).
It is intended to ship as a **bundled** TheiaOS extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.theiaos/extensions` as a standalone plugin directory.
