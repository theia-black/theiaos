...
metadata: { "theiaos": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == TheiaOS `sessions_spawn`
- **File I/O** == TheiaOS `read`/`write`
- **Remote fetch** == TheiaOS `web_fetch` (or `exec` with curl when POST is required)
