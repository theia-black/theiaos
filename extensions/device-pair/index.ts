...
import { approveDevicePairing, listDevicePairing } from "theiaos/plugin-sdk";
...
function resolveGatewayPort(cfg: TheiaOSPluginApi["config"]): number {
  const envRaw =
    process.env.THEIAOS_GATEWAY_PORT?.trim() || process.env.CLAWDBOT_GATEWAY_PORT?.trim();
  if (envRaw) {
...
async function resolveTailnetHost(api: TheiaOSPluginApi): Promise<string | null> {
...
function resolveAuth(cfg: TheiaOSPluginApi["config"]): ResolveAuthResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    process.env.THEIAOS_GATEWAY_TOKEN?.trim() ||
    process.env.CLAWDBOT_GATEWAY_TOKEN?.trim() ||
    cfg.gateway?.auth?.token?.trim();
  const password =
    process.env.THEIAOS_GATEWAY_PASSWORD?.trim() ||
...
async function resolveGatewayUrl(api: TheiaOSPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the TheiaOS iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the TheiaOS iOS app:",
            "",
            "```",
            qrAscii,
            "```",
