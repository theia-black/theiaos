...
import type { TheiaOSPluginApi, TheiaOSPluginService } from "theiaos/plugin-sdk";
...
function normalizeDenyList(cfg: TheiaOSPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: TheiaOSPluginApi["config"]): string[] {
...
export default function register(api: TheiaOSPluginApi) {
  ...
  const timerService: TheiaOSPluginService = {
