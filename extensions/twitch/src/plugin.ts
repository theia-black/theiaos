/**
 * Twitch channel plugin for TheiaOS.
 *
...
import { buildChannelConfigSchema } from "theiaos/plugin-sdk";
 ...
 * for TheiaOS. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: TheiaOSConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: TheiaOSConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: TheiaOSConfig): boolean => {
