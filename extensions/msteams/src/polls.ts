...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["theiaosPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["theiaos", "pollId"]) ??
    readNestedString(value, ["theiaos", "poll", "id"]) ??
    readNestedString(value, ["data", "theiaosPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "theiaos", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          theiaosPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "theiaos poll vote",
          displayText: "Vote recorded",
          value: { theiaosPollId: pollId, pollId },
        },
      },
    ],
  };
