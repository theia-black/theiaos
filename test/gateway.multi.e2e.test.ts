...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `theiaos-e2e-${name}-`));
  const configDir = path.join(homeDir, ".theiaos");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          THEIAOS_SKIP_BROWSER_CONTROL_SERVER: "1",
