...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    theiaosHome: process.env.THEIAOS_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("THEIAOS_HOME", snapshot.theiaosHome);
  restoreKey("THEIAOS_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked THEIAOS_HOME.
  delete process.env.THEIAOS_HOME;
  process.env.THEIAOS_STATE_DIR = path.join(base, ".theiaos");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "theiaos-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".theiaos", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
