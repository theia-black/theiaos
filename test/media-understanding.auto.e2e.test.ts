...
import type { TheiaOSConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("theiaos-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("theiaos-bin-sherpa-");
      const modelDir = await makeTempDir("theiaos-sherpa-model-");
      ...
      const cfg: TheiaOSConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("theiaos-bin-whispercpp-");
      const modelDir = await makeTempDir("theiaos-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("theiaos-bin-gemini-");
      ...
      const cfg: TheiaOSConfig = { tools: { media: { image: {} } } };
