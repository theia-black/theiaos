...
struct ChatMessageBubble: View {
    let message: TheiaOSChatMessage
    let style: TheiaOSChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? TheiaOSChatTheme.userText : TheiaOSChatTheme.assistantText
    ...
    private var inlineAttachments: [TheiaOSChatMessageContent] {
    ...
    private var toolCalls: [TheiaOSChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [TheiaOSChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? TheiaOSChatTheme.userBubble
        }
        if self.style == .onboarding {
            return TheiaOSChatTheme.onboardingAssistantBubble
        }
        return TheiaOSChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return TheiaOSChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: TheiaOSChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? TheiaOSChatTheme.userText : TheiaOSChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: TheiaOSChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(TheiaOSChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(TheiaOSChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [TheiaOSChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: TheiaOSChatTheme.assistantText)
            }
        }
    }
}
