...
enum TheiaOSChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("TheiaOSChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("TheiaOSChatTheme.onboardingAssistantBubble"),
...
enum TheiaOSPlatformImageFactory {
    static func image(_ image: TheiaOSPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
