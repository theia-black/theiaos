...
private let chatUILogger = Logger(subsystem: "ai.theiaos", category: "TheiaOSChatUI")

@MainActor
@Observable
public final class TheiaOSChatViewModel {
    public private(set) var messages: [TheiaOSChatMessage] = []
    ...
    public var attachments: [TheiaOSPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [TheiaOSChatPendingToolCall] = []
    public private(set) var sessions: [TheiaOSChatSessionEntry] = []
    private let transport: any TheiaOSChatTransport
    ...
    private var pendingToolCallsById: [String: TheiaOSChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any TheiaOSChatTransport) {
    ...
    public var sessionChoices: [TheiaOSChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [TheiaOSChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: TheiaOSPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [TheiaOSChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: TheiaOSChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func messageIdentityKey(for message: TheiaOSChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [TheiaOSChatMessage],
        incoming: [TheiaOSChatMessage]) -> [TheiaOSChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [TheiaOSChatMessage]) -> [TheiaOSChatMessage] {
        var result: [TheiaOSChatMessage] = []
    ...
    private static func dedupeKey(for message: TheiaOSChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [TheiaOSChatMessageContent] = [
            TheiaOSChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> TheiaOSChatAttachmentPayload in
            TheiaOSChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> TheiaOSChatSessionEntry {
        TheiaOSChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: TheiaOSChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: TheiaOSChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: TheiaOSAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = TheiaOSChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> TheiaOSPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
