...
@MainActor
struct TheiaOSChatComposer: View {
    @Bindable var viewModel: TheiaOSChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(TheiaOSChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(TheiaOSChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \TheiaOSPendingAttachment.id)
                { (att: TheiaOSPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            TheiaOSPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(TheiaOSChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(TheiaOSChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(TheiaOSChatTheme.subtleCard)
        .clipShape(Capsule())
    }
