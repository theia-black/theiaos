...
private final class TestChatTransport: @unchecked Sendable, TheiaOSChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [TheiaOSChatHistoryPayload]
    private let sessionsResponses: [TheiaOSChatSessionsListResponse]

    private let stream: AsyncStream<TheiaOSChatTransportEvent>
    private let continuation: AsyncStream<TheiaOSChatTransportEvent>.Continuation

    init(
        historyResponses: [TheiaOSChatHistoryPayload],
        sessionsResponses: [TheiaOSChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<TheiaOSChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> TheiaOSChatHistoryPayload {
        ...
        return self.historyResponses.last ?? TheiaOSChatHistoryPayload(
        ...
        attachments _: [TheiaOSChatAttachmentPayload]) async throws -> TheiaOSChatSendResponse
    {
        ...
        return TheiaOSChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> TheiaOSChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? TheiaOSChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: TheiaOSChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = TheiaOSChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = TheiaOSChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { TheiaOSChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = TheiaOSChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = TheiaOSChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = TheiaOSChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { TheiaOSChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
