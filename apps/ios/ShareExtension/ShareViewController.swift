...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.theiaos.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to TheiaOS", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to TheiaOS gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to TheiaOS.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "TheiaOSShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "TheiaOS is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "TheiaOS Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("theiaos-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
