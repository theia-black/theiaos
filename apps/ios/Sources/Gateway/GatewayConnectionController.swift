...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [TheiaOSCapability.canvas.rawValue, TheiaOSCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(TheiaOSCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(TheiaOSCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = TheiaOSLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(TheiaOSCapability.location.rawValue) }

        caps.append(TheiaOSCapability.device.rawValue)
        caps.append(TheiaOSCapability.photos.rawValue)
        caps.append(TheiaOSCapability.contacts.rawValue)
        caps.append(TheiaOSCapability.calendar.rawValue)
        caps.append(TheiaOSCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(TheiaOSCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            TheiaOSScreenCommand.record.rawValue,
            TheiaOSSystemCommand.notify.rawValue,
            TheiaOSChatCommand.push.rawValue,
        ...
        if caps.contains(TheiaOSCapability.camera.rawValue) {
            commands.append(TheiaOSCameraCommand.list.rawValue)
            commands.append(TheiaOSCameraCommand.snap.rawValue)
            commands.append(TheiaOSCameraCommand.clip.rawValue)
        }
        if caps.contains(TheiaOSCapability.location.rawValue) {
            commands.append(TheiaOSLocationCommand.get.rawValue)
        }
        if caps.contains(TheiaOSCapability.device.rawValue) {
            commands.append(TheiaOSDeviceCommand.status.rawValue)
            commands.append(TheiaOSDeviceCommand.info.rawValue)
        }
        if caps.contains(TheiaOSCapability.photos.rawValue) {
            commands.append(TheiaOSPhotosCommand.latest.rawValue)
        }
        if caps.contains(TheiaOSCapability.contacts.rawValue) {
            commands.append(TheiaOSContactsCommand.search.rawValue)
            commands.append(TheiaOSContactsCommand.add.rawValue)
        }
        if caps.contains(TheiaOSCapability.calendar.rawValue) {
            commands.append(TheiaOSCalendarCommand.events.rawValue)
            commands.append(TheiaOSCalendarCommand.add.rawValue)
        }
        if caps.contains(TheiaOSCapability.reminders.rawValue) {
            commands.append(TheiaOSRemindersCommand.list.rawValue)
            commands.append(TheiaOSRemindersCommand.add.rawValue)
        }
        if caps.contains(TheiaOSCapability.motion.rawValue) {
            commands.append(TheiaOSMotionCommand.activity.rawValue)
            commands.append(TheiaOSMotionCommand.pedometer.rawValue)
        }
