...
final class RemindersService: RemindersServicing {
    func list(params: TheiaOSRemindersListParams) async throws -> TheiaOSRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[TheiaOSReminderPayload], Error>) in
        ...
        return TheiaOSRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: TheiaOSRemindersAddParams) async throws -> TheiaOSRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = TheiaOSReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return TheiaOSRemindersAddPayload(reminder: payload)
    }
