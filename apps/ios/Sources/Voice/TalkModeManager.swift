...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__THEIAOS_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<TheiaOSTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = TheiaOSTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> TheiaOSTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return TheiaOSTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> TheiaOSTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> TheiaOSTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return TheiaOSTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> TheiaOSTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = TheiaOSTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: TheiaOSTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: TheiaOSAgentEventPayload.self) else {
                continue
            }
