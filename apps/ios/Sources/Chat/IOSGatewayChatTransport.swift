...
struct IOSGatewayChatTransport: TheiaOSChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.theiaos", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> TheiaOSChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(TheiaOSChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> TheiaOSChatHistoryPayload {
        ...
        return try JSONDecoder().decode(TheiaOSChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [TheiaOSChatAttachmentPayload]) async throws -> TheiaOSChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [TheiaOSChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(TheiaOSChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(TheiaOSGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: TheiaOSGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: TheiaOSChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: TheiaOSAgentEventPayload.self)
                        {
