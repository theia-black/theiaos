...
final class MotionService: MotionServicing {
    func activities(params: TheiaOSMotionActivityParams) async throws -> TheiaOSMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[TheiaOSMotionActivityEntry], Error>) in
        ...
        return TheiaOSMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: TheiaOSPedometerParams) async throws -> TheiaOSPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<TheiaOSPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = TheiaOSPedometerPayload(
