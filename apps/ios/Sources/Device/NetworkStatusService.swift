...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> TheiaOSNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> TheiaOSNetworkStatusPayload {
        let status: TheiaOSNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> TheiaOSNetworkStatusPayload {
        TheiaOSNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
