...
final class ContactsService: ContactsServicing {
    ...
    func search(params: TheiaOSContactsSearchParams) async throws -> TheiaOSContactsSearchPayload {
        ...
        return TheiaOSContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: TheiaOSContactsAddParams) async throws -> TheiaOSContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return TheiaOSContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return TheiaOSContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> TheiaOSContactPayload {
        TheiaOSContactPayload(
