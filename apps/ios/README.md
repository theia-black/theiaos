...
This is an **alpha** iOS app that connects to an TheiaOS Gateway as a `role: node`.
...
3. On iOS: TheiaOS → Settings → Gateway → paste setup code → Connect
...
- Otherwise it prefers the canonical TheiaOS team (`Y5PE65HELJ`) when that team exists locally.
...
cd apps/ios
xcodegen generate
xcodebuild test -project TheiaOS.xcodeproj -scheme TheiaOS -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/TheiaOSKit` contains the shared transport/types used by the iOS app.
