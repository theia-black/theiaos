...
private let webChatSwiftLogger = Logger(subsystem: "ai.theiaos", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: TheiaOSChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> TheiaOSChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(TheiaOSChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> TheiaOSChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                TheiaOSGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<TheiaOSChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any TheiaOSChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = TheiaOSChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: TheiaOSChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<TheiaOSChatView>) -> NSViewController
    {
