...
enum CommandResolver {
    private static let projectRootDefaultsKey = "theiaos.gatewayProjectRootPath"
    private static let helperName = "theiaos"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let theiaosEntry = root.appendingPathComponent("theiaos.mjs").path
        if FileManager().isReadableFile(atPath: theiaosEntry) { return theiaosEntry }
        let binEntry = root.appendingPathComponent("bin/theiaos.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/theiaos")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let theiaosPaths = self.theiaosManagedPaths(home: home)
        if !theiaosPaths.isEmpty {
            extras.insert(contentsOf: theiaosPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + theiaosPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func theiaosManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".theiaos"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func theiaosExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectTheiaOSExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("theiaos.mjs").path,
            root.appendingPathComponent("bin/theiaos.js").path,
        ]
    ...
    static func hasAnyTheiaOSInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.theiaosExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func theiaosNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let theiaosPath = self.projectTheiaOSExecutable(projectRoot: root) {
                return [theiaosPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "theiaos", subcommand] + extraArgs
            }
            if let theiaosPath = self.theiaosExecutable(searchPaths: searchPaths) {
                return [theiaosPath, subcommand] + extraArgs
            }

            let missingEntry = """
            theiaos entrypoint missing (looked for dist/index.js or theiaos.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real theiaos CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/theiaos"
        ...
        \(cliSection)
        \(projectSection)
        if command -v theiaos >/dev/null 2>&1; then
          CLI="$(command -v theiaos)"
          theiaos \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/theiaos.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/theiaos.mjs"
            node "$PRJ/theiaos.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/theiaos.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/theiaos.js"
            node "$PRJ/bin/theiaos.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent theiaos"
          pnpm --silent theiaos \(quotedArgs);
        else
          echo "theiaos CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? TheiaOSConfigFile.loadDict()
