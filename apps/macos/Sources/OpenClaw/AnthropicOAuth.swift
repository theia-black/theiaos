...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (TheiaOS token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: TheiaOSOAuthStore.AnthropicOAuthStatus = TheiaOSOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.theiaos", category: "anthropic-oauth")
...
enum TheiaOSOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let theiaosOAuthDirEnv = "THEIAOS_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "TheiaOS OAuth token file not found"
            case .unreadableFile: "TheiaOS OAuth token file not readable"
            case .invalidJSON: "TheiaOS OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in TheiaOS OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "TheiaOS OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.theiaosOAuthDirEnv]?
        ...
        return home.appendingPathComponent(".theiaos", isDirectory: true)
