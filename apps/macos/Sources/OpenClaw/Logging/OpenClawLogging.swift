...
enum TheiaOSLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = TheiaOSOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = TheiaOSFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        TheiaOSLogging.bootstrapIfNeeded()
        let label = TheiaOSLogging.makeLabel(subsystem: subsystem, category: category)
...
struct TheiaOSOSLogHandler: LogHandler {
...
struct TheiaOSFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = TheiaOSLogging.parseLabel(self.label)
