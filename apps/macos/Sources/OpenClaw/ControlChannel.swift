...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: TheiaOSProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.theiaos", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: TheiaOSKit.AnyCodable]()) {
                $0[$1.key] = TheiaOSKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: TheiaOSProtocol.AnyCodable?) -> [String: TheiaOSProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: TheiaOSProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: TheiaOSKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: TheiaOSProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("theiaos.control.heartbeat")
    static let controlAgentEvent = Notification.Name("theiaos.control.agent")
}
