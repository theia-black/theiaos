...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.theiaos.mac`) even if the shipped
    ...
    {
        if environment["THEIAOS_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "theiaos.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "theiaos.nixMode") { return true }
