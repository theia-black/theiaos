...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.theiaos", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> TheiaOSSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: TheiaOSSessionPreviewEntry,
    ...
    private static func cache(payload: TheiaOSSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: TheiaOSChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [TheiaOSKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [TheiaOSKit.AnyCodable]) -> [TheiaOSChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(TheiaOSChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: TheiaOSChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: TheiaOSChatMessage) -> Bool {
    ...
    private static func toolNames(for message: TheiaOSChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: TheiaOSChatMessage) -> String? {
