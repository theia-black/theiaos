...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.theiaos/theiaos.json. Prefer THEIAOS_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.theiaos/theiaos.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.theiaos/theiaos.json. Restart the gateway to apply."
        }
