...
enum DebugActions {
    private static let verboseDefaultsKey = "theiaos.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "theiaos.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = TheiaOSPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "TheiaOS", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = TheiaOSPaths.configURL
        guard
