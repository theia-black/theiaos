...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: TheiaOSOAuthStore.AnthropicOAuthStatus = TheiaOSOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: TheiaOSOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = TheiaOSOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = TheiaOSOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try TheiaOSOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. TheiaOS can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: TheiaOSOAuthStore.AnthropicOAuthStatus,
