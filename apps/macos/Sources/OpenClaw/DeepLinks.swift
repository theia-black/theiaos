...
private let deepLinkLogger = Logger(subsystem: "ai.theiaos", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "TheiaOS is paused", message: "Unpause TheiaOS to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run TheiaOS agent?", message: body) else { return }
        }
