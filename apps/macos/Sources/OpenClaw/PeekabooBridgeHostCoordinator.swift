...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.theiaos", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: TheiaOSPeekabooBridgeServices?
    private static var theiaosSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("TheiaOS", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = TheiaOSPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.theiaosSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.theiaosSocketPath, privacy: .public)")
    }
...
@MainActor
private final class TheiaOSPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.theiaos.peekaboo")
