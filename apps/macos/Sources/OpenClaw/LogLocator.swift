...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["THEIAOS_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/theiaos")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("theiaos-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("theiaos-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/theiaos/ (rolling or stdout), or nil if none exist.
