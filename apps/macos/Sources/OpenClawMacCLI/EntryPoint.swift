...
struct TheiaOSMacCLI {
    static func main() async {
        ...
        default:
            fputs("theiaos-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      theiaos-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      theiaos-mac discover [--timeout <ms>] [--json] [--include-local]
      theiaos-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      theiaos-mac connect
      theiaos-mac connect --url ws://127.0.0.1:18789 --json
      theiaos-mac discover --timeout 3000 --json
      theiaos-mac wizard --mode local
    """)
}
