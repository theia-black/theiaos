...
@Suite struct VoiceWakeRuntimeTests {
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["theiaos"]
        let text = "hey theiaos"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey theiaos do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["theiaos"], minPostTriggerGap: 0.3)
