...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: TheiaOSProtocol.AnyCodable(["ok": TheiaOSProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: TheiaOSProtocol.AnyCodable(["ok": TheiaOSProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = TheiaOSProtocol.AnyCodable([
            "runId": TheiaOSProtocol.AnyCodable("run-1"),
            "sessionKey": TheiaOSProtocol.AnyCodable("main"),
            "state": TheiaOSProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: TheiaOSProtocol.AnyCodable(["a": TheiaOSProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
