...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: TheiaOSProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: TheiaOSProtocol.AnyCodable] = [
            "presence": .init([TheiaOSProtocol.AnyCodable(entry)]),
        ]
        let payload = TheiaOSProtocol.AnyCodable(payloadMap)
