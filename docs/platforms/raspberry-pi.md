---
summary: "TheiaOS on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up TheiaOS on a Raspberry Pi
  - Running TheiaOS on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# TheiaOS on Raspberry Pi

## Goal

Run a persistent, always-on TheiaOS Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most TheiaOS features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled theiaos

# Enable if not
sudo systemctl enable theiaos

# Start on boot
sudo systemctl start theiaos
```
...
```bash
# Check logs
journalctl -u theiaos --no-pager -n 100

# Common fix: rebuild
cd ~/theiaos  # if using hackable install
npm run build
sudo systemctl restart theiaos
```
