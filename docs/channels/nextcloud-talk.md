...
```bash
theiaos plugins install @theiaos/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
theiaos plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "TheiaOS" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `theiaos pairing list nextcloud-talk`
  - `theiaos pairing approve nextcloud-talk <CODE>`
