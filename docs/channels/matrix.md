...
Matrix is an open, decentralized messaging protocol. TheiaOS connects as a Matrix **user**
...
```bash
theiaos plugins install @theiaos/matrix
```

Local checkout (when running from a git repo):

```bash
theiaos plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `theiaos plugins install @theiaos/matrix`
   - From a local checkout: `theiaos plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: TheiaOS calls the same
     login endpoint, stores the access token in `~/.theiaos/credentials/matrix/credentials.json`,
...
- On first connection, TheiaOS requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  TheiaOS logs a warning.
...
`~/.theiaos/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `theiaos pairing list matrix`
  - `theiaos pairing approve matrix <CODE>`
...
- On startup, TheiaOS resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
