...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects TheiaOS to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
theiaos plugins install @theiaos/feishu
```

Local checkout (when running from a git repo):

```bash
theiaos plugins install ./extensions/feishu
```
...
If you just installed TheiaOS, run the wizard:
...
2. Configuring app credentials in TheiaOS
...
1. You already ran `theiaos channels add` for Feishu
2. The gateway is running (`theiaos gateway status`)
...
```bash
theiaos pairing approve feishu <CODE>
```
...
2. Run `theiaos logs --follow` and look for `chat_id`
...
2. Run `theiaos logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `theiaos gateway status`  | Show gateway status           |
| `theiaos gateway install` | Install/start gateway service |
| `theiaos gateway stop`    | Stop gateway service          |
| `theiaos gateway restart` | Restart gateway service       |
| `theiaos logs --follow`   | Tail gateway logs             |
...
4. Check logs: `theiaos logs --follow`
...
5. Ensure the gateway is running: `theiaos gateway status`
6. Check logs: `theiaos logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.theiaos/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.theiaos/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
