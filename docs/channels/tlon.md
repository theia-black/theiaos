...
Tlon is a decentralized messenger built on Urbit. TheiaOS connects to your Urbit ship and can
...
```bash
theiaos plugins install @theiaos/tlon
```

Local checkout (when running from a git repo):

```bash
theiaos plugins install ./extensions/tlon
```
...
By default, TheiaOS blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `theiaos message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, TheiaOS replies in-thread.
