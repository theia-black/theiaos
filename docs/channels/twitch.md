...
read_when:
  - Setting up Twitch chat integration for TheiaOS
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. TheiaOS connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
theiaos plugins install @theiaos/twitch
```

Local checkout (when running from a git repo):

```bash
theiaos plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `THEIAOS_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "theiaos", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use THEIAOS_TWITCH_ACCESS_TOKEN env var)
...
```bash
THEIAOS_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
