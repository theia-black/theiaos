...
read_when:
  - You want TheiaOS to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables TheiaOS to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`theiaos onboard`) and `theiaos channels add` list optional channel plugins.
...
```bash
theiaos plugins install @theiaos/nostr
```

Use a local checkout (dev workflows):

```bash
theiaos plugins install --link <path-to-theiaos>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "theiaos@example.com",
        "lud16": "theiaos@example.com"
      }
    }
  }
}
