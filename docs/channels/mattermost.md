---
summary: "Mattermost bot setup and TheiaOS config"
read_when:
...
```bash
theiaos plugins install @theiaos/mattermost
```

Local checkout (when running from a git repo):

```bash
theiaos plugins install ./extensions/mattermost
```
...
4. Configure TheiaOS and start the gateway.
...
- Approve via:
  - `theiaos pairing list mattermost`
  - `theiaos pairing approve mattermost <CODE>`
...
Use these target formats with `theiaos message send` or cron/webhooks:
