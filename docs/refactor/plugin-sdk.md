...
- Publish as `theiaos/plugin-sdk` (or export from core under `theiaos/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `TheiaOSPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: TheiaOSConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: TheiaOSConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: TheiaOSConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: TheiaOSConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: TheiaOSConfig): string;
  };
};
```
...
- Introduce `theiaos/plugin-sdk`.
- Add `api.runtime` to `TheiaOSPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `theiaosRuntime: ">=2026.2.0"`).
