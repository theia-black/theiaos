...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.theiaos/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.theiaos/node.json` (node id + pairing token).
...
Path: `~/.theiaos/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.theiaos/exec-approvals.sock",
...
- Unix socket at `~/.theiaos/exec-approvals.sock` (0600).
