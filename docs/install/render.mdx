...
Deploy TheiaOS on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/theia-black/theiaos)
...
repository configures everything needed to run TheiaOS:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: THEIAOS_STATE_DIR
        value: /data/.theiaos
      - key: THEIAOS_WORKSPACE_DIR
        value: /data/workspace
      - key: THEIAOS_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/theiaos`.
...
If you use the original TheiaOS repository, Render will not auto-deploy your TheiaOS. To update it, run a manual Blueprint sync from the dashboard.
...
For TheiaOS, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any TheiaOS host.
