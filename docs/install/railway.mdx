...
Deploy TheiaOS on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/theiaos` — Control UI
...
- `THEIAOS_STATE_DIR=/data/.theiaos` (recommended)
- `THEIAOS_WORKSPACE_DIR=/data/workspace` (recommended)
- `THEIAOS_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your TheiaOS state + workspace so you can migrate to another host without losing config or memory.
