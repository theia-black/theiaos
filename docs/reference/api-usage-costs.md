...
TheiaOS features that can generate provider usage or paid API calls.
...
- `theiaos status --usage` and `theiaos channels list` show provider **usage windows**
...
TheiaOS can pick up credentials from:
...
`theiaos models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
