...
TheiaOS integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- TheiaOS owns lifecycle when `channels.signal.autoStart=true`.
...
- TheiaOS spawns `imsg rpc` as a child process (legacy iMessage integration).
