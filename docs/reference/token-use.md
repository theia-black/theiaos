---
summary: "How TheiaOS builds prompt context and reports token usage + costs"
read_when:
...
TheiaOS tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

TheiaOS assembles its own system prompt on every run. It includes:
...
For images, TheiaOS downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from TheiaOS session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `theiaos status --usage` and `theiaos channels list` show
...
`cacheWrite`. If pricing is missing, TheiaOS shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. TheiaOS can
...
Anthropic's 1M context window is currently beta-gated. TheiaOS can inject the
