...
This document explains how TheiaOS manages sessions end-to-end:
...
TheiaOS is designed around a single **Gateway process** that owns session state.
...
TheiaOS persists sessions in two layers:
...
- Transcripts: `~/.theiaos/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

TheiaOS resolves these via `src/config/sessions.ts`.
...
TheiaOS intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (TheiaOS consumes the events, but Pi decides when to compact).
...
TheiaOS also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, TheiaOS bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, TheiaOS leaves it alone.
...
- `theiaos status` (CLI)
- `theiaos sessions` / `sessions --json`
...
TheiaOS supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- TheiaOS strips/suppresses this in the delivery layer.

As of `2026.1.10`, TheiaOS also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
TheiaOS uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but TheiaOS’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `theiaos status`.
