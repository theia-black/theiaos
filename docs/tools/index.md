---
summary: "Agent tool surface for TheiaOS (browser, canvas, nodes, message, cron) replacing legacy `theiaos-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `theiaos-*` skills
title: "Tools"
---

# Tools (TheiaOS)

TheiaOS exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `theiaos-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `theiaos.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, TheiaOS logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:theiaos`: all built-in TheiaOS tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`theiaos node run`).
...
TheiaOS tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `theiaos configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated TheiaOS-managed browser.
...
- Quick smoke: `theiaos nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `theiaos gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, TheiaOS clamps `tools.sessions.visibility` to `tree`.
