...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `theiaos sandbox explain`.
...
Legacy `agent.*` configs are migrated by `theiaos doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=theiaos-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${THEIAOS_STATE_DIR:-$HOME/.theiaos}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
