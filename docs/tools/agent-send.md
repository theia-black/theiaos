---
summary: "Direct `theiaos agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `theiaos agent` (direct agent runs)

`theiaos agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `theiaos message --target`).
...
```bash
theiaos agent --to +15555550123 --message "status update"
...
theiaos agent --to +15555550123 --message "Summon reply" --deliver
