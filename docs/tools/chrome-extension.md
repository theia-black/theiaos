---
summary: "Chrome extension: let TheiaOS drive your existing Chrome tab"
read_when:
...
The TheiaOS Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate theiaos-managed Chrome profile.
...
TheiaOS then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the TheiaOS release (npm package) as static files. There is no separate “build” step.

After upgrading TheiaOS:

- Re-run `theiaos browser extension install` to refresh the installed files under your TheiaOS state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

TheiaOS ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want TheiaOS to control.
...
- `ON`: attached; TheiaOS can drive that tab.
...
Debugging: `theiaos sandbox explain`
...
`theiaos browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `theiaos browser extension install` first to copy the extension to a stable location under your TheiaOS state directory.
...
- **This is not isolated** like the dedicated theiaos-managed profile.
