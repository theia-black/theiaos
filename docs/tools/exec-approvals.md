...
- **gateway host** → `theiaos` process on the gateway machine
...
`~/.theiaos/exec-approvals.json` directly.

CLI: `theiaos approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
