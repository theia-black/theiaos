...
- `~/Library/Application Support/TheiaOS/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `theiaos-canvas://<session>/<path>`

示例：

- `theiaos-canvas://main/` → `<canvasRoot>/main/index.html`
- `theiaos-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `theiaos-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
theiaos nodes canvas present --node <id>
theiaos nodes canvas navigate --node <id> --url "/"
theiaos nodes canvas eval --node <id> --js "document.title"
theiaos nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__theiaos__/a2ui/
```
...
theiaos nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
theiaos nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "theiaos://agent?message=Review%20this%20design";
```
