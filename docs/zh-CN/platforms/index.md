...
TheiaOS 核心使用 TypeScript 编写。**Node 是推荐的运行时**。
...
- 服务状态：`theiaos gateway status`

## Gateway 网关服务安装（CLI）

使用以下任一方式（均支持）：

- 向导（推荐）：`theiaos onboard --install-daemon`
- 直接安装：`theiaos gateway install`
- 配置流程：`theiaos configure` → 选择 **Gateway service**
- 修复/迁移：`theiaos doctor`（提供安装或修复服务）

服务目标取决于操作系统：

- macOS：LaunchAgent（`bot.molt.gateway` 或 `bot.molt.<profile>`；旧版 `com.theiaos.*`）
- Linux/WSL2：systemd 用户服务（`theiaos-gateway[-<profile>].service`）
