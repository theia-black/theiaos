...
你可以在 `~/.theiaos/theiaos.json` 中覆盖此设置：
...
所有日志配置都在 `~/.theiaos/theiaos.json` 的 `logging` 下。
...
- TheiaOS 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `theiaos plugins enable diagnostics-otel` 启用插件。
...
- `theiaos.tokens`（计数器，属性：`theiaos.token`、`theiaos.channel`、`theiaos.provider`、`theiaos.model`）
- `theiaos.cost.usd`（计数器，属性：`theiaos.channel`、`theiaos.provider`、`theiaos.model`）
- `theiaos.run.duration_ms`（直方图，属性：`theiaos.channel`、`theiaos.provider`、`theiaos.model`）
- `theiaos.context.tokens`（直方图，属性：`theiaos.context`、`theiaos.channel`、`theiaos.provider`、`theiaos.model`）

消息流：

- `theiaos.webhook.received`（计数器，属性：`theiaos.channel`、`theiaos.webhook`）
- `theiaos.webhook.error`（计数器，属性：`theiaos.channel`、`theiaos.webhook`）
- `theiaos.webhook.duration_ms`（直方图，属性：`theiaos.channel`、`theiaos.webhook`）
- `theiaos.message.queued`（计数器，属性：`theiaos.channel`、`theiaos.source`）
- `theiaos.message.processed`（计数器，属性：`theiaos.channel`、`theiaos.outcome`）
- `theiaos.message.duration_ms`（直方图，属性：`theiaos.channel`、`theiaos.outcome`）

队列 + 会话：

- `theiaos.queue.lane.enqueue`（计数器，属性：`theiaos.lane`）
- `theiaos.queue.lane.dequeue`（计数器，属性：`theiaos.lane`）
- `theiaos.queue.depth`（直方图，属性：`theiaos.lane` 或 `theiaos.channel=heartbeat`）
- `theiaos.queue.wait_ms`（直方图，属性：`theiaos.lane`）
- `theiaos.session.state`（计数器，属性：`theiaos.state`、`theiaos.reason`）
- `theiaos.session.stuck`（计数器，属性：`theiaos.state`）
- `theiaos.session.stuck_age_ms`（直方图，属性：`theiaos.state`）
- `theiaos.run.attempt`（计数器，属性：`theiaos.attempt`）
...
- `theiaos.webhook.error`
  - `theiaos.channel`、`theiaos.webhook`、`theiaos.chatId`、`theiaos.error`
- `theiaos.message.processed`
  - `theiaos.channel`、`theiaos.outcome`、`theiaos.chatId`、`theiaos.messageId`、`theiaos.sessionKey`、`theiaos.sessionId`、`theiaos.reason`
- `theiaos.session.stuck`
  - `theiaos.state`、`theiaos.ageMs`、`theiaos.queueDepth`、`theiaos.sessionKey`、`theiaos.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `theiaos doctor`。
