...
- Jobs persist under `~/.theiaos/cron/` so restarts don’t lose schedules.
...
theiaos cron list
theiaos cron run <job-id>
theiaos cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.theiaos/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `theiaos cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, TheiaOS applies a
...
If `delivery` is omitted for isolated jobs, TheiaOS defaults to `announce`.
...
- Job store: `~/.theiaos/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.theiaos/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
theiaos cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
theiaos cron edit <jobId> --agent ops
theiaos cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
theiaos cron run <jobId>
theiaos cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
theiaos cron edit <jobId> \
...
```bash
theiaos system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`theiaos system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `THEIAOS_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- TheiaOS applies exponential retry backoff for recurring jobs after consecutive errors:
