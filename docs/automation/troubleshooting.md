...
theiaos system heartbeat last
theiaos logs --follow
theiaos config get agents.defaults.heartbeat
theiaos channels status --probe
```
...
```bash
theiaos config get agents.defaults.heartbeat.activeHours
theiaos config get agents.defaults.heartbeat.activeHours.timezone
theiaos config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
theiaos cron list
theiaos logs --follow
```
