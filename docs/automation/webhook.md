...
read_when:
  ...
  - Wiring external systems into TheiaOS
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your TheiaOS config directory (typically `~/.theiaos/hooks/transforms`).
...
- `theiaos webhooks gmail setup` writes `hooks.gmail` config for `theiaos webhooks gmail run`.
