...
```bash
theiaos tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `theiaos tui --deliver`
...
- Check the Gateway logs: `theiaos logs --follow`.
- Confirm the agent can run: `theiaos status` and `theiaos models status`.
...
- No agents in picker: check `theiaos agents list` and your routing config.
