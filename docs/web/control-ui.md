...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/theiaos`)
...
If the page fails to load, start the Gateway first: `theiaos gateway`.
...
you revoke it with `theiaos devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.theiaos/theiaos.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. TheiaOS
...
TheiaOS **blocks** Control UI connections without device identity.
