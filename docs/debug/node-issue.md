...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in TheiaOS
...
Running TheiaOS via Node with `tsx` fails at startup with:

```
[theiaos] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `theiaos status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch theiaos.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node theiaos.mjs status` works on Node 25.
