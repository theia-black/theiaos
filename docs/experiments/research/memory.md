...
read_when:
  - Designing workspace memory (~/.theiaos/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep TheiaOS integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.theiaos/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.theiaos/workspace` as canonical human-readable memory.
...
```
~/.theiaos/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in TheiaOS**, but keep a separable core library.

### Why integrate into TheiaOS?

- TheiaOS already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `theiaos memory recall "…" --k 25 --since 30d`
  - `theiaos memory reflect --since 7d`
...
Pragmatic take for `~/.theiaos/workspace`:
