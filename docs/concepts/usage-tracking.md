...
- `/usage cost` in chats: local cost summary aggregated from TheiaOS session logs.
- CLI: `theiaos status --usage` prints a full per-provider breakdown.
- CLI: `theiaos channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
