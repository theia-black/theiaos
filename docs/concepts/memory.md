---
title: "Memory"
summary: "How TheiaOS memory works (workspace files + automatic memory flush)"
read_when:
...
TheiaOS memory is **plain Markdown in the agent workspace**. The files are the
...
`~/.theiaos/workspace`). See [Agent workspace](/concepts/agent-workspace) for the full layout.
...
When a session is **close to auto-compaction**, TheiaOS triggers a **silent,
...
TheiaOS can build a small vector index over `MEMORY.md` and `memory/*.md` so
...
- Configure memory search under `agents.defaults.memorySearch` (not top-level
  `memorySearch`).
- Uses remote embeddings by default. If `memorySearch.provider` is not set, TheiaOS auto-selects:
...
Remote embeddings **require** an API key for the embedding provider. TheiaOS
...
BM25 + vectors + reranking. Markdown stays the source of truth; TheiaOS shells
...
- The gateway runs QMD in a self-contained XDG home under
  `~/.theiaos/agents/<agentId>/qmd/` by setting `XDG_CONFIG_HOME` and
  `XDG_CACHE_HOME`.
...
- The gateway writes a self-contained QMD home under
  `~/.theiaos/agents/<agentId>/qmd/` (config + cache + sqlite DB).
...
- Searches run via `memory.qmd.searchMode` (default `qmd search --json`; also
  ...
  QMD build, TheiaOS retries with `qmd query`. If QMD fails or the binary is
  missing, TheiaOS automatically falls back to the builtin SQLite manager so
  memory tools keep working.
- TheiaOS does not expose QMD embed batch-size tuning today; batch behavior is
  controlled by QMD itself.
- **First search may be slow**: QMD may download local GGUF models (reranker/query
  expansion) on the first `qmd query` run.
  - TheiaOS sets `XDG_CONFIG_HOME`/`XDG_CACHE_HOME` automatically when it runs QMD.
  - If you want to pre-download models manually (and warm the same index TheiaOS
    uses), run a one-off query with the agent’s XDG dirs.

    TheiaOS’s QMD state lives under your **state dir** (defaults to `~/.theiaos`).
    You can point `qmd` at the exact same index by exporting the same XDG vars
    TheiaOS uses:

    ```bash
    # Pick the same state dir TheiaOS uses
    STATE_DIR="${THEIAOS_STATE_DIR:-$HOME/.theiaos}"
...
- When `scope` denies a search, TheiaOS logs a warning with the derived
  `channel`/`chatType` so empty results are easier to debug.
- Snippets sourced outside the workspace show up as
  `qmd/<collection>/<relative-path>` in `memory_search` results; `memory_get`
  understands that prefix and reads from the configured QMD collection root.
- When `memory.qmd.sessions.enabled = true`, TheiaOS exports sanitized session
  transcripts (User/Assistant turns) into a dedicated QMD collection under
  `~/.theiaos/agents/<id>/qmd/sessions/`, so `memory_search` can recall recent
...
- Index storage: per-agent SQLite at `~/.theiaos/memory/<agentId>.sqlite` (configurable via `agents.defaults.memorySearch.store.path`, supports `{agentId}` token).
- Freshness: watcher on `MEMORY.md` + `memory/` marks the index dirty (debounce 1.5s). Sync is scheduled on session start, on search, or on an interval and runs asynchronously. Session transcripts use delta thresholds to trigger background sync.
- Reindex triggers: the index stores the embedding **provider/model + endpoint fingerprint + chunking params**. If any of those change, TheiaOS automatically resets and reindexes the entire store.

### Hybrid search (BM25 + vector)

When enabled, TheiaOS combines:

- **Vector similarity** (semantic match, wording can differ)
- **BM25 keyword relevance** (exact tokens like IDs, env vars, code symbols)

If full-text search is unavailable on your platform, TheiaOS falls back to vector-only search.
...
TheiaOS can cache **chunk embeddings** in SQLite so reindexing and frequent updates (especially session transcripts) don't re-embed unchanged text.
...
- Session logs live on disk (`~/.theiaos/agents/<agentId>/sessions/*.jsonl`). Any process/user with filesystem access can read them, so treat disk access as the trust boundary. For stricter isolation, run agents under separate OS users or hosts.
...
When the sqlite-vec extension is available, TheiaOS stores embeddings in a
...
- If the sqlite-vec extension is missing or fails to load, TheiaOS logs the
