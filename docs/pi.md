...
This document describes how TheiaOS integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

TheiaOS uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, TheiaOS directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in TheiaOS's local TUI mode)                                             |
...
├── pi-tools.ts                    # createTheiaOSCodingTools()
...
├── theiaos-tools.ts              # TheiaOS-specific tools
...
2. **Custom Replacements**: TheiaOS replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **TheiaOS Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures TheiaOS's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, TheiaOS CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
TheiaOS wraps this with `guardSessionManager()` for tool result safety.
...
TheiaOS maintains an auth profile store with multiple API keys per provider:
...
TheiaOS loads custom pi extensions for specialized behavior:
...
TheiaOS also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | TheiaOS Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom TheiaOS tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.theiaos/agents/<agentId>/sessions/` (or `$THEIAOS_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-tools.create-theiaos-coding-tools.adds-claude-style-aliases-schemas-without-dropping-b.test.ts`
- `src/agents/pi-tools.create-theiaos-coding-tools.adds-claude-style-aliases-schemas-without-dropping-d.test.ts`
- `src/agents/pi-tools.create-theiaos-coding-tools.adds-claude-style-aliases-schemas-without-dropping-f.test.ts`
- `src/agents/pi-tools.create-theiaos-coding-tools.adds-claude-style-aliases-schemas-without-dropping.test.ts`
