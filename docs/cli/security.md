---
summary: "CLI reference for `theiaos security` (audit and fix common security footguns)"
read_when:
...
```bash
theiaos security audit --json | jq '.summary'
theiaos security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
theiaos security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
