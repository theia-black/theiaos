...
description: "Complete field-by-field reference for ~/.theiaos/theiaos.json"
---

# Configuration Reference

Every field available in `~/.theiaos/theiaos.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — TheiaOS uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `theiaos doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["theiaos-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `theiaos plugins install @theiaos/mattermost`.
...
TheiaOS spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@theiaos"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `theiaos.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, TheiaOS auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, TheiaOS assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.theiaos/sandboxes`
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.theiaos/workspace",
        agentDir: "~/.theiaos/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@theiaos"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:theiaos`   | All built-in tools (excludes provider plugins)                                           |
...
TheiaOS uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.theiaos/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `THEIAOS_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `theiaos onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `theiaos onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `theiaos onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `theiaos onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `theiaos onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `theiaos onboard --auth-choice minimax-api`.
...
- Loaded from `~/.theiaos/extensions`, `<workspace>/.theiaos/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or THEIAOS_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `THEIAOS_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.theiaos-dev` + port `19001`), `--profile <name>` (uses `~/.theiaos-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.theiaos/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-theiaos-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `THEIAOS_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or THEIAOS_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__theiaos__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__theiaos__/a2ui/`
...
- Also serves A2UI at `/__theiaos__/a2ui/`.
...
- Hostname defaults to `theiaos`. Override with `THEIAOS_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.theiaos/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `theiaos dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.theiaos/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${THEIAOS_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.theiaos/credentials/oauth.json`.
...
- Default log file: `/tmp/theiaos/theiaos-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `theiaos doctor --fix` can strip unknown keys).
