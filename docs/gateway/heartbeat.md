...
- During heartbeat runs, TheiaOS treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `theiaos sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, TheiaOS skips the heartbeat run entirely (no model call).
...
```bash
theiaos system event --text "Check for urgent follow-ups" --mode now
```
