---
summary: "Use Venice AI privacy-focused models in TheiaOS"
read_when:
  - You want privacy-focused inference in TheiaOS
...
```bash
theiaos onboard --auth-choice venice-api-key
```
...
```bash
theiaos chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, TheiaOS shows all available Venice models. Pick based on your needs:
...
```bash
theiaos models set venice/claude-opus-45
theiaos models set venice/llama-3.3-70b
```

List all available models:

```bash
theiaos models list | grep venice
```

## Configure via `theiaos configure`

1. Run `theiaos configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
TheiaOS automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
theiaos chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
theiaos chat --model venice/claude-opus-45

# Use uncensored model
theiaos chat --model venice/venice-uncensored

# Use vision model with image
theiaos chat --model venice/qwen3-vl-235b-a22b

# Use coding model
theiaos chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `theiaos models list` to see currently available models. Some models may be temporarily offline.
