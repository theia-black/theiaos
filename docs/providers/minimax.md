---
summary: "Use MiniMax M2.1 in TheiaOS"
read_when:
  - You want MiniMax models in TheiaOS
...
```bash
theiaos plugins enable minimax-portal-auth  # skip if already loaded.
theiaos gateway restart  # restart if gateway is already running
theiaos onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/theia-black/theiaos/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `theiaos.json`:
...
- Use `theiaos models list` and `theiaos models set minimax/MiniMax-M2.1` to switch.
...
- Running `theiaos configure` and selecting **MiniMax M2.1**, or
