---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for TheiaOS deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, TheiaOS cron scheduling for periodic checks, or version status checks on a machine running TheiaOS (laptop, workstation, Pi, VPS).
---

# TheiaOS Host Hardening

## Overview

Assess and harden the host running TheiaOS, then align it to a user-defined risk tolerance without breaking access. Use TheiaOS security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim TheiaOS changes the host firewall, SSH, or OS updates; it does not.
...
5. TheiaOS gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if TheiaOS can access sensitive data.
...
### 2) Run TheiaOS security audits (read-only)

As part of the default read-only checks, run `theiaos security audit --deep`. Only offer alternatives if the user requests them:

1. `theiaos security audit` (faster, non-probing)
2. `theiaos security audit --json` (structured output)

Offer to apply TheiaOS safe defaults (numbered):

1. `theiaos security audit --fix`

Be explicit that `--fix` only tightens TheiaOS defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check TheiaOS version/update status (read-only)

As part of the default read-only checks, run `theiaos update status`.
...
- Credential hygiene notes (location of TheiaOS creds, prefer disk encryption)
...
After TheiaOS install or first hardening pass, run at least one baseline audit and version check:

- `theiaos security audit`
- `theiaos security audit --deep`
- `theiaos update status`

Ongoing monitoring is recommended. Use the TheiaOS cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `theiaos cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `theiaos update status`
...
Before creating, `theiaos cron list` and match on exact `name`. If found, `theiaos cron edit <id> ...`.
If not found, `theiaos cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `theiaos update status` (preferred for source checkouts and channels)
2. `npm view theiaos version` (published npm version)

## TheiaOS command accuracy

Use only supported commands and flags:

- `theiaos security audit [--deep] [--fix] [--json]`
- `theiaos status` / `theiaos status --deep`
- `theiaos health --json`
- `theiaos update status`
- `theiaos cron add|list|runs|run`

Do not invent CLI flags or imply TheiaOS enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by TheiaOS compaction:
